...
本次目标：**L6c registry 终结 + 类型擦除**。**已完成 scope map + 设计 spec**（`docs/superpowers/specs/2026-05-31-ooc-4-L6c-registry-termination-design.md`，由 8 路并行 reader workflow + 对抗综合产出，已核验剔除 reader stale claim + Supervisor 拍板 4 开放点）。**先完整读该 spec**——它含：①核验校正（L6c 类型擦除**只剩 do+talk**，todo/plan/relation 早是 root 方法非 window type；behavior.ts **无** resolveOnClose/compressView 故接链是真前置；worker 已 talks.json 权威；deliverMessage 已存在；base/ 无 do/talk/feishu proto）②4 个 Supervisor 拍板（D1 custom 保留动态派生特例不上 base 链 / D2 feishu_* 独立 plugin registry / D3 删 do_window.move LLM 面保留 fork 糖 / D4 command_exec graceful 迁移）③依赖 DAG + 7 子增量拆分（推荐序 L6c-6 proto 吸收 → L6c-5 改名 → L6c-1 onClose/compressView 接链[high-risk async 传染]→ L6c-2 talk 擦除 → L6c-3 do 擦除 → L6c-4 registry 删[分 2 commit]→ L6c-7 前端+测试）④跨面冲突（service.ts/render.ts 被 4 面触碰须串行勿并行）⑤compressView⟂L8 解耦判定（L6c 不被 L8 阻塞）。**high-risk 子增量(1/2/3/4)先补回归基线 e2e（agent-to-agent-wait-wake/do-fork-and-collect/sharing）再动**。每子增量走 plan→feasibility review→执行→harness 回归。
