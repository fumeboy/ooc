...
> 实现位置：`packages/@ooc/meta/storybook/`。上游：`packages/@ooc/meta/engineering.testing.doc.ts`（e2e 策略）。
...
packages/@ooc/meta/storybook/
  ...
  specs/capability_<cap>.md         # 9 份能力规格（收编 playbook 场景 + 旧 test_object_*.md）
  runner.ts                         # 聚合 → 覆盖矩阵 + dashboard
```
