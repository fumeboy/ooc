...
| UI-6 | P2 · 信息呈现 | thinkloop trace | 三张 `notice LLM_INTERACTION · CALL_STARTED` 卡片近乎相同、各展开整段 JSON、挤占右栏 | 同类 notice 折叠/合并为「LLM 调用 ×N」一行摘要，JSON 默认收起 | ☐ 待修 |
| UI-7 | P2 · 样式统一 | 全程 | 中英混排系统性割裂（英文 heading + 中文正文同屏、按钮「暂停 session」「done」混用） | 定主语言，文案集中过一遍 i18n，至少同句不双语并排 | ☐ 待修 |
| UI-8 | P2 · 美观 | thread graph 画布 | 两节点占顶部一条，下方 ~70% 高度全空 | 节点垂直居中/自适应缩放填充 | ☐ 待修 |
| UI-9 | P3 · 友好度 | 窄屏 768px | 侧栏仍占 ~300px、不收起，两栏被压窄，无 hamburger/抽屉 | 断点收起侧栏为抽屉 | ☐ 待修 |
| UI-10 | P3 · 友好度 | 全程 | stone/flow/pool/thread/window/object 术语对新用户无 tooltip/glossary 入口 | 首屏或导航旁加轻量「这些词是什么」入口 | ☐ 待修 |
...
> P2/P3（UI-6~10）为 polish 项，本文跟踪，留后续一轮 UI polish 处理。
