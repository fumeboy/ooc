...
| `unactive`（新增，复用 dead `destruct?`） | session refcount **1 → 0** | 每次停用 | v1 仅 thread | 切 `canceled` + 级联停用子树（§3.3）；**返回 `{}`=保留为 canceled、不 delete** |

- `construct` 一次；`active`/`unactive` 重复。
- **`unactive` 返回 `{ delete?: boolean }`**（用户细化三）：缺省 / `false` = 只停用（释放运行时资源、磁盘身份留存，可再被引用而 `active`）；**`delete: true` = core 把该 object 彻底从 session 移除（含持久化文件）**。删除只在引用归零这一刻、由 object 自决——**无独立 destruct 钩子、无强制销毁**，故不会留悬空引用。仅 `unactive` 路径 honor `delete`；`active` 返回值忽略。
- **签名（非「与 construct 完全同构」）**：生命周期钩子作用于**既有**对象、不产 Data。v1 定 `exec(ctx) => void | { delete?: boolean }`，`ctx` 携带 `thread`（解引用发生处的线程）+ `targetId`（refcount 变动的对象 id）+ `runtime`；**钩子 body 自解析它要操作的对象**（§3.3）。core 派发器保持泛型、零 class policy。
...
- `dispatchUnactiveIfZero(ctxThread, targetId, targetClass, registry)`：
  ...
  3. 否则 `const r = await hook.exec({ thread: ctxThread, targetId, runtime })`——**单次调用**，body 自解析目标。core **不** import 任何 thread 符号、不 special-case `THREAD_CLASS_ID`。
  4. 若 `r?.delete === true` → core 把 targetId 彻底从 session 移除：删其持久化（默认 `objectDir`/`threadDir` 路径；自定义 persistable 经其 `delete?` 自理）+ 从内存持有处移除。**v1 无 builtin 返回 delete**（thread 返回 `{}`）；该路径经合成测试 class 验证、非死槽。
...
| 层 | 改动 | 锚点 |
|---|---|---|
| executable 契约 | 删 `ObjectDestructor`；加 `ObjectLifecycleHook`（`exec(ctx) => void \| { delete?: boolean }`）+ `LifecycleContext`（带 targetId） | `core/executable/contract.ts:192` |
| object-lifecycle 删除 | `dispatchUnactiveIfZero` 收到 `{delete:true}` → 删持久化（`objectDir`/`threadDir`，或 persistable `delete?`）+ 移除内存实例 | 新文件 `object-lifecycle.ts` |
...
- **v1 = `close` + 级联 `close` 触发 unactive；仅 fork 子线程；内存树 refcount；canceled + 子树级联；construct 标记不可关窗；`unactive` 返回 `{delete?}`、dispatch honor（合成测试验证，thread 不用 delete）。**
...
### 修订三（2026-06-21，用户细化：unactive 返回值）
