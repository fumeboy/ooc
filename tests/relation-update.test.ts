...
const RELATION_UPDATE_TRAIT_DIR = join(KERNEL_ROOT, "traits", "talkable", "relation_update");
...
describe("talkable/relation_update TRAIT.md", () => {
  test("TRAIT.md 存在且可被 loader 解析", async () => {
    expect(existsSync(join(RELATION_UPDATE_TRAIT_DIR, "TRAIT.md"))).toBe(true);
    const trait = await loadTrait(RELATION_UPDATE_TRAIT_DIR, "kernel");
  ...
  test("TRAIT.md 正文包含关键段落（发起方 / 接收方指导）", () => {
    const content = readFileSync(join(RELATION_UPDATE_TRAIT_DIR, "TRAIT.md"), "utf-8");
