...
import { readPlan } from "../../src/persistable";
import {
  countFormExecutions,
  hasLlmEnv,
  llm,
...
describe.skipIf(!hasLlmEnv)("integration: plan-then-execute", () => {
  ...
  test("agent makes a plan then executes via program_window", async () => {
    const root = await makeRootThread(
      tempRoot,
      [
        "请先用 open(command=\"plan_set\", title=\"...\", args={ content: \"- [ ] 数 src/ 下所有 .ts 文件总数\" }) 做一份计划（args 给齐时 open 会立即提交 form）。",
    ...
    // OOC-4 L5b: plan 塌缩为 owner flow plan.md（不再是 plan_window）
    const objectRef = root.persistence!;
    expect((await readPlan(objectRef)).length).toBeGreaterThan(0);
