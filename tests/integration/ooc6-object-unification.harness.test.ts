...
import { createFlowsService } from "../../src/app/server/modules/flows/service";
...
import {
  ...
  contextDir,
} from "../../src/persistable/flow-context";
import {
  listRegisteredWindowTypes,
  getWindowTypeDefinition,
} from "../../src/executable/windows";
import { createStoneObject, writeReadable, writeReadme } from "../../src/persistable";
import { renderContextXml } from "../../src/thinkable/context/render";
import { makeThread } from "../../src/__tests__/make-thread";
import {
  parseTrigger,
  evaluateTrigger,
} from "../../src/thinkable/knowledge/triggers";
import { derivePeerObjectWindows } from "../../src/thinkable/knowledge/synthesizer";
import { WindowManager } from "../../src/executable/windows/_shared/manager";
...
describe("ooc-6 Object Unification harness cycle", () => {
  ...
  it("0+2: context windows are objects - thread holds unified ContextObject array", async () => {
    ...
    const customWindow = thread.contextWindows.find((w: any) => w.id === "custom:test_agent");
    expect(customWindow).toBeDefined();
    expect(customWindow.type).toBe("custom");
    expect(customWindow.objectId).toBe("test_agent");

    const doWindow = thread.contextWindows.find((w: any) => w.type === "do");
    ...
    const todo = thread.contextWindows.find((w: any) => w.id === todoId);
  ...
  it("1: builtin objects are registered from src/extendable/base/", () => {
    ...
    // All builtin types should be registered (12 core types)
    const expectedBuiltins = [
      "root", "command_exec", "todo", "file", "knowledge",
      "search", "skill_index", "plan", "program", "custom",
      "do", "talk",
    ];
    for (const t of expectedBuiltins) {
      expect(types).toContain(t);
      const def = getWindowTypeDefinition(t as any);
  ...
  it("4: readable concept - objects render their readable content in XML", async () => {
    ...
    // Insert custom window for the readable_test object
    const mgr = WindowManager.fromThread(thread);
    mgr.insertTypedWindow({
      id: "w_custom_readable",
      type: "custom",
      parentWindowId: "root",
      title: "Readable Test",
      status: "active",
      createdAt: Date.now(),
      objectId: "readable_test",
      content: "", // prevent undefined content in XML render
    }, thread);
    thread.contextWindows = mgr.toData();
  ...
  it("7: each object type has its own methods (commands) registered", () => {
    ...
    // Custom should have edit_relation command
    const customDef = getWindowTypeDefinition("custom");
    expect(customDef.commands["edit_relation"]).toBeDefined();
  ...
  it("9: context objects persist to flows/<sid>/objects/<oid>/context/<cid>/window.json", async () => {
    ...
    expect(readBack.path).toBe("test/persist");
    ...
    mgr3.removeWindow(windowId, thread);
  ...
  it("10: peer agent stone automatically appears as context object (no relation window needed)", async () => {
    ...
    const peer = peerWindows.find((w) => w.objectId === peerId);
    expect(peer).toBeDefined();
    expect(peer!.type).toBe("custom");
    expect(peer!.title).toBe("Peer Agent"); // from readme title

    // Verify no relation window is needed - peer appears as custom window
    const relationWindows = peerWindows.filter((w) => (w as any).type === "relation");
