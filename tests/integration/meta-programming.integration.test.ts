  ...
  test("agent registers a method then calls it", async () => {
    ...
    const { inbox, events } = bootstrapInboxFromPrompt(
      [
        ...
        "",
        "协议要求：",
        "1) exec(method=\"program\", description=\"...\") 只创建 form，不等于执行。",
        "2) 业务参数必须放在 open.args 或 refine.args，不要把 language/code/window_id/method 写进 description。",
        "3) 只有在参数齐全后，才 submit(form_id)。",
        "",
        "Step 1: 用 program method（language=shell）写文件，",
        ...
        "",
        "Step 2: 用 program method（window_id='custom:agent', method='add', args={a:7,b:8}）调用该 method。",
