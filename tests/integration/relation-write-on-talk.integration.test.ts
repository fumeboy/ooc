 ...
 *         存在且非空;LLM 用 write_file 创建(从轨迹可见 toolName=exec,
...
import { processQueuedJobs } from "../../src/app/server/runtime/worker";
  ...
  test("assistant writes relation file after talk with critic", async () => {
    ...
    const writeFileCalls = events.filter(
      (e) =>
        ...
        e.toolName === "exec" &&
        e.ok &&
        // exec 的 arguments 含 method:"write_file";由于 output 是字符串,稍微宽松一点匹配
