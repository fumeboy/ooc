...
/* ========================================================================
 * Symptom 1: do(wait=true) 后，awaitingChildren 含子线程 ID
 * ======================================================================== */
...
describe("Symptom 1 — engine 集成: 子线程 title 与 submit.title 一致", () => {
  test("do(wait=true) 创建的子线程 title 与 submit 参数一致", async () => {
    /* 父线程：open(do) → submit(wait=true, title="分析 G3 基因")
    ...
    /* 整体：父线程应进入 waiting 或 failed（不是因为 do 本身报错） */
...
/* ========================================================================
 * 组合验证：do(wait=true) 后父线程 awaitingChildren 填充正确
 * （通过 engine 集成路径验证 Symptom 1 的根本修复）
 * ======================================================================== */

describe("Symptom 1 — engine 集成: do(wait=true) 填充 awaitingChildren", () => {
