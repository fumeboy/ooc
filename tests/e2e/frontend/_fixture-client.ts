  ...
  world: async ({}, use) => {
    ...
    const fixture: WorldFixture = {
      ...
      createStone(objectId) {
        const dir = join(baseDir, "stones", objectId);
      ...
      writeStoneClient(objectId, code) {
        const file = join(baseDir, "stones", objectId, "client", "index.tsx");
        mkdirSync(join(baseDir, "stones", objectId, "client"), { recursive: true });
        writeFileSync(file, code, "utf8");
      },
      writeFlowClientPage({ sessionId, objectId, page, code }) {
        const dir = join(baseDir, "flows", sessionId, "objects", objectId, "client", "pages");
        mkdirSync(dir, { recursive: true });
