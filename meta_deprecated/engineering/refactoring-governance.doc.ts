...
/**
 * RefactoringGovernance 概念：复杂度治理的长期工程规范。
 *
 * 治理目标不是"把文件拆小"，而是持续降低偶然复杂度。
 *
 ...
 */
export type RefactoringGovernanceConcept = Concept & {
  sources: {
    flowsService: typeof flowsService;
    threadTransition: typeof threadTransition;
    thinkableContext: typeof thinkableContext;
    programCommand: typeof programCommand;
  ...
  stateGovernance: {
    title: string;
    summary?: string;
    /** 推荐做法：transition helper + query helper 分工 */
    recommended: DocNode;
    /** 状态散落的坏味道 */
    badSmells: InvariantNode;
  };
  ...
  directoryShape: {
    ...
    summary?: string;
    /** 同名目录升级触发条件 */
    sameNameDirUpgrade: DocNode;
    /** command 子系统目录拆分模式 */
    commandSubsystem: DocNode;
  };
  ...
  testGate: {
    ...
    tddMinimum: DocNode;
    /** 重构优先补的 4 类测试 */
    prioritizedTypes: DocNode;
    /** 避免的低价值测试 */
    antiPatterns: DocNode;
  };
  ...
  verificationGate: {
    ...
    summary?: string;
    /** 至少验证的 4 项 */
    minimumCoverage: DocNode;
    /** 推荐验证顺序 */
    suggestedOrder: DocNode;
    /** "应该可以"不能代替结果 */
    freshEvidenceRequired: InvariantNode;
  };

  /** 按语义边界拆 commit */
  commitGranularity: DocNode;

  /** 文档同步规范：与 meta-doc-maintenance 的接口 */
  docSync: {
    title: string;
    summary?: string;
    /** 哪些重构改动需要同步文档 */
...
export const refactoring_governance_v20260512_1: RefactoringGovernanceConcept = {
...
目标不是"把文件拆小"，而是持续降低偶然复杂度，让系统更容易理解、验证、演进。
`.trim(),

  goal: {
    title: "重构目标：减少三类复杂度",
    summary: "概念混居 / 状态散落 / 副作用边界模糊 三类",
    content: `
- **概念混居**：一个实现同时承载多个并列概念，调用方必须理解全部内部细节才能
  安全修改
- **状态语义分散**：同一个状态转换规则散落在多个模块里，以"补分支"而不是
  "改规则"的方式演进
- **副作用边界模糊**：目录扫描、状态翻转、入队、网络请求、协议兼容等副作用
  在同一处混写
...
如果一次重构没有显著降低这三类复杂度，它通常只是代码搬运，而不是有效治理。
  ...
  fileSplit: {
    title: "文件拆分原则",
    summary: "按主干 / 旁生概念划分，不按行数 / 函数数",

    principles: {
      title: "主干 vs 旁生",
      content: `
- **主干概念**：构成模块主阅读路径的核心职责，应集中在稳定入口
- **旁生概念**：围绕主干生长、但会打扰主路径理解的投影 / 适配 / 格式化 / 协议
  提示 / 兼容逻辑
...
样板：\`src/thinkable/context/\`（主干在 \`index.ts\`，render/knowledge/protocol
下沉）、\`src/executable/windows/root/program.ts\`（command 入口保留，执行
细节迁入 \`src/executable/program/\`）。
      `.trim(),
    },
  },
  ...
  stateGovernance: {
    title: "状态语义治理",
    summary: "thread / job / form / pause / resume 类生命周期状态必须集中表达",
...
样板：\`src/app/server/runtime/thread-transition.ts\` 与
\`src/app/server/runtime/thread-query.ts\` 把状态规则从 flowsService 下沉出来。
      `.trim(),
    },
...
- 目录扫描 + 状态判断 + 入队逻辑堆在 HTTP service 中
      `.trim(),
      rationale: `
状态规则散落会让一次合理修改必须同时改 N 处，少改一处就成隐 bug；集中表达
后规则演化只在一处，service 层不再担心状态语义，仅消费转换结果。
      `.trim(),
    },
  },
...
- command 文件本身只保留 KNOWLEDGE / path / match / 入口分发 / 参数兜底
  ...
  testGate: {
    title: "测试门禁",
    summary: "测试先行；没看见测试先失败，不能声称验证了重构价值",
...
5. 最后再决定是否 commit / push
    ...
    freshEvidenceRequired: {
      kind: "invariant",
      title: "'应该可以'不能代替结果",
      summary: "没重新跑验证命令就声称'通过'是被禁止的失同步状态",
      content: `
- 没重新跑验证命令就声称"通过"
...
- 验证失败时继续提交，期待后续再补
      `.trim(),
      rationale: `
不带新鲜证据的"通过"声明会绕过门禁；一旦放行，错误会以"应该没问题"形式
反复滚雪球，最终在最不该出问题的位置爆掉。
      `.trim(),
    },
  },
  ...
  commitGranularity: {
    title: "提交粒度",
    summary: "按语义边界拆 commit，而不是按今天改了哪些文件",
    content: `
推荐粒度：
...
不推荐：把状态迁移 / 目录搬家 / 文档更新 / provider 改造全塞进一个不可审阅
的大提交。
    `.trim(),
  },
  ...
  docSync: {
    title: "文档同步规范",
    summary: "改了目录结构 / 准则 / 心智模型 / 验证方式即需同步文档",
...
3. 历史结果性文档只做最小必要修正，不大面积回填
    ...
    machineGate: {
      title: "机器闸落地",
      content: `
具体落地：meta 文档同步靠 \`metaDocMaintenance\` 规范的机器闸（\`bun tsc
--noEmit\` + \`bun test meta/__tests__\`），不靠纪律。任何重构在合规下都会
顺带跑一遍 meta 一致性检查。

约束反过来：源码改名 / 删除 → 对应 .doc.{js,ts} 的 \`import * as ns\` 立即
tsc 失败。失同步状态在门禁层即被拦下。
      `.trim(),
    },
  },
