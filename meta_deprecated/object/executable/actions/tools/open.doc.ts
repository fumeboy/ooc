...
/**
 * open 概念：LLM "开始一次行动 / 加载一个资源到 Context" 的统一入口。
 *
 * sources:
 *  - open — handleOpenTool 实现（type=command / knowledge / file 三分支）
 */
export type OpenConcept = Concept & {
  sources: { open: typeof openSource };

  /** type=command / knowledge / file 三种调用分支 */
  typeBranches: {
    title: string;
    summary?: string;
    /** 创建可执行 form */
    typeCommand: DocNode;
    /** 挂 knowledge 到 Context */
    typeKnowledge: DocNode;
    /** 挂 file 到 Context */
    typeFile: DocNode;
  };

  /** todo 不走独立 type，统一走 type=command 入口 */
  todoEntry: DocNode;
  ...
  universalParams: {
    ...
    /** 为本次行动提供人类可读标签 */
    titleParam: DocNode;
    /** 把新 window 挂为已有 window 的 sub-window */
    parentWindowIdParam: DocNode;
    /** mark 附加参数（详见 mark 概念） */
    markParam: DocNode;
    /** deps 决策来源声明 */
    depsParam: DocNode;
  };
...
export const open_v20260506_1: OpenConcept = {
  name: "Open",
  get parent() {
...
`.trim(),
  ...
  typeBranches: {
    title: "type 分支",
    summary: "command 创建可执行 form；knowledge / file 只是把资源挂入 Context",

    typeCommand: {
      title: "type=command",
      summary: "创建 command_exec window 并激活 command path 对应的 knowledge",
      content: `
\`\`\`
open(
  type="command",
...
grep）在 args 给齐时 open 立即提交 form，无需再 refine/submit。
    ...
    typeKnowledge: {
      ...
      summary: "挂一个 type=knowledge 的 window 作为显式 pin 路径",
      content: `
\`\`\`
open(
  type="knowledge",
...
适用场景：临时想查阅某篇 knowledge 全文，与当前 form 的 command 无关。
    ...
    typeFile: {
      title: "type=file",
      summary: "挂一个 type=file 的 window，渲染时按 path 读取正文",
      content: `
\`\`\`
open(
  type="file",
...
- lines / columns 作为元数据保留；当前渲染层尚未按窗口裁剪
  ...
  todoEntry: {
    title: "todo 入口",
    summary: "todo 不走独立 type=todo，统一走 command 入口；args 齐全时立即提交",
    content: `
...
后续完成 / 撤销通过 close(window_id="<todo_window_id>")。
  ...
  universalParams: {
    title: "通用附加参数",
    summary: "与 type / command 正交的顶层参数",
...
talk_window 上调用其注册的 say command。
    ...
    markParam: {
      title: "mark",
      content: `
标记 inbox 消息（详见 mark 概念）。任意 tool 调用都可携带。
      `.trim(),
    },
...
任意 tool 调用都可携带。
  ...
  returnValue: {
    title: "返回值",
    summary: "command 返回 form_id；knowledge / file 返回 window id",
    content: `
