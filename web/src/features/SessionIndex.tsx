...
import { talkTo, fetchFlow, fetchSessions, fetchObjects, pauseObject, resumeFlow, updateSessionTitle } from "../api/client";
...
import { Send, X, Plus, BarChart3 } from "lucide-react";
...
export function SessionIndex({ sessionId }: SessionIndexProps) {
  ...
  const commitTitle = () => {
    ...
    if (trimmed && trimmed !== sessionTitle && activeId) {
      updateSessionTitle(activeId, trimmed).catch(console.error);
      if (activeFlow) setActiveFlow({ ...activeFlow, title: trimmed });
    } else {
      ...
      if (activeId) {
        fetchFlow(activeId).then((serverFlow) => {
          setActiveFlow((prev) => {
            ...
            /* 合并消息：保留本地已有但服务器尚未返回的消息 */
            const serverTimestamps = new Set(serverFlow.messages.map(m => `${m.timestamp}-${m.from}-${m.content?.slice(0, 50)}`));
            const localOnly = prev.messages.filter(m =>
              !serverTimestamps.has(`${m.timestamp}-${m.from}-${m.content?.slice(0, 50)}`)
            );
            return {
              ...serverFlow,
              messages: [...serverFlow.messages, ...localOnly],
            };
          });
        }).catch(console.error);
      }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 shrink-0">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0 mx-3">
          {editingTitle ? (
            <input
              ...
              className="flex-1 min-w-0 bg-transparent text-sm font-medium text-[var(--foreground)] outline-none border-b border-[var(--border)]"
            />
          ) : (
            <button
              onClick={() => setEditingTitle(true)}
              className="text-sm font-medium text-[var(--foreground)] hover:text-[var(--muted-foreground)] truncate text-left transition-colors"
        ...
        <div className="flex items-center gap-1">
          {/* 新建 session */}
          <button
            onClick={() => { setActiveId(null); setActiveFlow(null); }}
      ...
      {showGantt ? (
        <div className="flex-1 overflow-auto">
          <SessionGantt sessionId={sessionId} />
        </div>
      ) : (
        <>
          {/* 消息列表 */}
          ...
          {/* 输入框 */}
          <div className="px-4 pb-3 pt-2 shrink-0 relative">
            {showMention && mentionCandidates.length > 0 && (
              <div
                ref={mentionRef}
                className="absolute bottom-full left-4 right-4 mb-1 rounded-lg bg-[var(--popover)] border border-[var(--border)] shadow-lg overflow-hidden z-10"
              >
