/**
 * ThreadNode — thread item in the StaffView grid.
 *
 * Row layout:
 *   [status dot] [thread title] [session-entry badge?] [t·N chips] [→ context btn]
 *
 * Clicking the row selects the thread (writes ?objectId=&threadId= to URL).
 * The → button navigates to /flows/thread_context view.
 */
...
interface ThreadNodeProps {
  ...
  level: number;
  active: boolean;
  /** user.root rows cannot be switched to; disabled=true mutes click + styles */
  disabled?: boolean;
  onSelect: () => void;
}
...
export function ThreadNode({
  ...
  return (
    ...
    >
      ...
      {isUserRoot && (
        <span className="thread-node-badge thread-node-badge-entry" title="session entry — cannot switch view">
          session entry
        </span>
      )}
