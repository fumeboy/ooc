...
export async function fetchSelfFirstLine(objectId: string): Promise<string | null> {
  ...
  try {
    // ooc-3: path is /api/stones/main/:name/self; returns "content" field
    const res = await requestJson<{ content?: string; text?: string }>(
      `/api/stones/main/${encodeURIComponent(objectId)}/self`,
    );
    const text = typeof res?.content === "string" ? res.content : (typeof res?.text === "string" ? res.text : "");
    if (!text) return null;
...
async function loadReadme(objectId: string): Promise<string | null> {
  ...
  const p = (async () => {
    ...
    if (!isLikelyStoneObjectId(objectId)) {
      ...
      try {
        // ooc-3: path is /api/stones/main/:name/readme; returns "content" field
        const res = await requestJson<{ content?: string; text?: string }>(
          `/api/stones/main/${encodeURIComponent(objectId)}/readme`,
        );
        text = typeof res?.content === "string" ? res.content : (typeof res?.text === "string" ? res.text : null);
      } catch {
        text = null;
      }
    }
