...
describe("parseOocUri", () => {
  it("maps flow page form to SPA route", () => {
    expect(
      parseOocUri("ooc://client/flows/s_xyz/objects/alan/pages/report-2026"),
    ).toBe("/flows/s_xyz/objects/alan/pages/report-2026");
  });
  ...
  it("re-encodes segments containing special chars", () => {
    expect(
      parseOocUri("ooc://client/flows/s 1/objects/a b/pages/p c"),
    ).toBe("/flows/s%201/objects/a%20b/pages/p%20c");
  });

  it("returns null for unknown ooc client shape (wrong arity)", () => {
    expect(parseOocUri("ooc://client/flows/s_xyz/objects/alan")).toBeNull();
  });

  it("returns null for non-client ooc scheme", () => {
    expect(parseOocUri("ooc://world/stones/user")).toBeNull();
    expect(parseOocUri("ooc://server/flows/s/objects/a/pages/p")).toBeNull();
  });
