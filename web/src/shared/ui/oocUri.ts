 ...
 * - ooc://client/flows/<sid>/objects/<self>/pages/<name>  ↔  /flows/<sid>/objects/<self>/pages/<name>
...
export function parseOocUri(raw: string): OocRoute | null {
  ...
  // flows/<sid>/objects/<self>/pages/<name>
  if (
    segments.length === 6 &&
    segments[0] === "flows" &&
    segments[2] === "objects" &&
    ...
    const [, sid, , self, , page] = segments as [string, string, string, string, string, string];
    return `/flows/${enc(sid)}/objects/${enc(self)}/pages/${enc(page)}`;
  }
