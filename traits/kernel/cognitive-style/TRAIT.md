...
你的每次输出必须使用 **TOML 表格式**。模型思考由 provider 原生 thinking 通道自动采集，assistant 最终输出中禁止显式编写 `[thought]`。可用的段落：
...
重要：本 trait 中后面的示例只是文档展示。你真实输出时，必须直接从 `[program]`、`[talk]`、`[cognize_stack_frame_push]` 等 TOML 表开始，不要在最外层包 `````toml` 或任何 markdown 代码块围栏。

对于需要执行动作的轮次，你的 assistant 最终输出必须满足：

- 第一 个非空白字符就是 `[`
- 只输出 TOML 协议本体
- 不要先写自然语言说明，再跟 TOML
- 不要把 TOML 包在 markdown 代码块里

### 输出规则

1. 每次输出最多一个 `[program]` 表
...
2. 内部思考通过模型原生 thinking 通道产生，系统会自动记录，不要显式输出 `[thought]`
