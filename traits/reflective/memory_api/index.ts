...
export const llm_methods: Record<string, TraitMethod> = {
  query_memory: {
    name: "query_memory",
    description:
      "查询长期记忆条目（结构化 memory/entries/*.json）。按 query（模糊匹配 key/content/tags/category） + tags + since + pinned + limit 过滤，返回精简摘要列表（含 contentPreview 200 字符截断）。不返回过期条目（除非 includeExpired=true）。",
    params: [
      { name: "query", type: "string", description: "模糊匹配关键词", required: false },
