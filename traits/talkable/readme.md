...
直接在输出中使用 `[talk/目标对象名]` 段落，消息内容会被流式推送：
...
规则：
- `[talk/目标]` 必须独占一行，目标名只允许字母、数字、下划线、连字符
- `[/talk]` 结束标记必须独占一行
- 可以在一次输出中包含多个 `[talk/xxx]` 段落（发给不同对象）
- `[talk]` 和 `[program]` 互斥 —— 同一次输出中不能同时使用两者
- `[talk]` 可以和 `[thought]`、`[finish]`、`[wait]` 并存
- 向人类回复用 `[talk/user]`，`[talk/human]` 也可以
...
收到消息时，系统会显示消息 ID（如 `#msg_xxx`）。你可以用第三个参数指定回复哪条消息：
...
2. 同时告知完整报告的位置（writeShared 的文件名）
