...
/**
 * IntegrationTests 概念：真 LLM 端到端集成测试的策略 / fixture / 清单 / 已修复 bug。
 *
 * sources（被本规范测试集合覆盖的核心 loop）:
 *  - scheduler  — 多 thread 调度与唤醒机制
 *  - thinkloop  — 单 thread 内 LLM → tool 调度循环
 *  - executable — 5 原语 + window / command 注册中枢
 */
export type IntegrationTestsConcept = Concept & {
  sources: {
  ...
  testInventory: {
    ...
    summary?: string;
    /** 单 thread 基础 4 个测试 */
    singleThreadBasic: ExampleNode;
    /** form 生命周期 2 个测试 */
    formLifecycle: ExampleNode;
    /** 多任务编排 2 个测试 */
    multiTaskOrchestration: ExampleNode;
    /** 多线程 2 个测试 */
  ...
  bugsCaught: {
    title: string;
    summary?: string;
    /** Claude SSE 解析与 input_json_delta */
    claudeSseFallback: DocNode;
    /** tool_use 文本回放污染 transcript */
    toolUseTextReplay: DocNode;
    /** await_children 唤醒后看不到子结果 */
    awaitChildrenBlind: DocNode;
    /** fork 子线程出生时 transcript 空 */
    forkChildEmptyTranscript: DocNode;
    /** plan command args 形态过严 */
    planArgShape: DocNode;
  };

  /** 写新集成测试时应回答的 5 个问题 */
  authoringGuide: DocNode;

  /** 跑法：env 控制 + 命令样例 */
  runHowTo: ExampleNode;
};
...
export const integration_tests_v20260511_1: IntegrationTestsConcept = {
...
单元测试覆盖内部算法、错误路径、组件契约；集成测试在此之上做"真 LLM 端到端
能否跑通"的硬验证。二者目的互补：单测保算法正确性，集成测保系统作为整体在
真实 LLM 行为下不崩。
`.trim(),
...
| 数字断言 | 用区间 / 包含 而非确定值 | LLM 偶尔把数字写错一位的容错 |
  ...
  fixture: {
    kind: "example",
    title: "通用 fixture",
    summary: "tests/integration/_fixture.ts 暴露的 5 个 helper",
    content: `
\`\`\`ts
...
| 6 | wait-state-transition | wait tool / status=waiting + inboxSnapshotAtWait 已写入 |
      `.trim(),
    },
...
只可能出现在 U3 合成的 \`<knowledge_window>\` 标签里。
  ...
  bugsCaught: {
    title: "已触发并修复的真实 bug",
    summary: "集成测试抓出过用单元测试根本不可能发现的 5 个真实问题",

    claudeSseFallback: {
      title: "Claude 代理只返回 SSE，generateWithClaude 解析失败",
      content: `
...
- **顺带修复**：streamWithClaude 原本只在 content_block_start 时取 input，对 Anthropic 标准的 input_json_delta 增量协议无效，tool 参数一直为空。补上按 index 缓冲 partial_json + content_block_stop 时 JSON.parse
    ...
    toolUseTextReplay: {
      title: "tool_use 文本回放导致 LLM 输出假 tool call",
      content: `
...
- **修复**：tool_use 事件不进 transcript（commit 021a66f）。原则：对 LLM 不重要的事件不要塞进 conversation history，用 system context 表达当前状态
    ...
    awaitChildrenBlind: {
      title: "父线程 await_children 醒来后看不到子结果",
      content: `
...
- **修复**：scheduler 在唤醒父线程时主动 inject \`[await_children] 等待中的子线程已完成: ...\` 事件（commit cccd3fb），父醒来就知道哪个子线程完成、它的 endSummary 是什么
    ...
    forkChildEmptyTranscript: {
      title: "fork 创建的子线程出生时 transcript 全空被 Anthropic 拒绝",
      content: `
...
- **修复**：do.fork 创建 child 时同步把初始消息以 \`[初始消息] xxx\` inject 形式入 child events（commit 10b94eb）
    ...
    planArgShape: {
      title: "plan command 对 LLM 自然 args 形态过严",
      content: `
