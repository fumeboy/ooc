...
作为常驻 knowledge 出现在 Context，说明：

- tool \`open / refine / submit / close / wait / compress\` 的用法
- mark 机制
- 基本的 form 生命周期约束
`.trim(),
  },
...
和普通 knowledge 一样按需激活，由 command 路径触发。

\`\`\`
能力层（按需激活）
  ├── kernel:executable      代码执行（触发：open(type=command, command=program, ...)）
  ├── kernel:collaborable    对象间通信（触发 command：talk）
  ├── kernel:reflectable     反思与沉淀（描述如何把经历沉淀为长期 knowledge / 元编程）
  ├── kernel:plannable       任务规划（触发 command：do / plan）
  └── kernel:compress        上下文压缩（描述如何审视 process events，标记冗余区段）
\`\`\`
`.trim(),
  },
