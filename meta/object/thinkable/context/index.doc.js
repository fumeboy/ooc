...
- **todo**：由 root.todo command 一步直建的可见待办（args 给齐时 open 立即提交 form）
...
现在 todo_window 是独立 window 类型，由 \`open(command="todo", title=..., args={ content, on_command_path? })\`
在 args 给齐时一步直建。完成时 \`close(window_id="<todo_window_id>", reason=...)\`。
