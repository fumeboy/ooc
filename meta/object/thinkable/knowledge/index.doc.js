...
- **激活规则**：activates_on.show_content_when 与 union(所有 \`thread.contextWindows\` 中 type=command_exec 的 commandPaths) ∩ ≠ ∅ → full；show_description_when 命中 → summary；同时命中 full 优先
- **手动 pin**：Step 1 已暂停（spec 2026-05-14 § 迁移节奏 Step 2 才回归 knowledge_window）；持久化反序列化时旧 \`pinnedKnowledge\` 字段会被丢弃并 warn
- **热重载**：loader 按 "文件路径+mtime" 签名缓存，Agent 编辑 .md 后下一轮 think 立即生效
- **上限**：单篇 full 内容 8KB 截断；激活集合超 20 项截尾

补充当前实现细节：

- 当前 activator 的顺序：根据 contextWindows 中 command_exec 子集的 commandPaths 计算 union，先放 full 命中，再补 summary 命中，整体做去重与 20 项上限。
- pinned knowledge 概念在 Step 2 通过 knowledge_window 重新引入；目前不存在显式 pin 通道。
