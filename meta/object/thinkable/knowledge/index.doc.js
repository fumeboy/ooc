...
每篇 knowledge = 一个 markdown 文档 + yaml frontmatter：
...
也可以手动激活：LLM 调用 \`open(type=knowledge, path=...)\` 显式加载某篇 knowledge；
...
- \`relations/{objectId}.md\` — 描述与某个具体其他 Object 的关系；由 talk 等协作过程
  自动维护或 Object 主动编辑。详见 collaborable/relation。
- \`memory/index.md\` — 长期记忆索引页，跨任务持久存在；通过 super 分身的 SuperFlow 写入
  （详见 reflectable）。进入 Context 的入口由 context-builder 控制（截尾上限保护）。
`.trim(),
  },
...
- 路径 ID = 相对 knowledge/ 的路径，不带 .md 后缀（例：\`build-tools/file-ops\` / \`memory/index\`）；
  frontmatter \`filename\` 字段作文档化字段，以文件路径为准
