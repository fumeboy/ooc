...
import * as context from "@src/thinkable/context";
 ...
 *  - context     — buildInputItems 读 self.md 注入 LLM instructions + 渲染 <self object_id>
 */
...
自己通过 reflectable 通道精简 self.md。
    ...
    injection: {
      title: "Context 注入",
      content: `
每一轮 ThinkLoop 时 self.md 沿两条通道进入 LLM：

1. **instructions 通道**：self.md 正文整篇作为 LlmGenerateParams.instructions 注入
   （由 thinkable.context.buildInputItems 通过 sources.stoneSelf.readSelf 加载）；
   provider 适配层（openai / claude）原样透传到顶层 system instructions
2. **XML 标记通道**：thinkable.context 的 system XML 顶部渲染 \`<self object_id="…">\`，
   让 Object 在多方 Session 中明确"我是谁"

注入是无条件的（与 knowledge 的渐进式激活不同）。Object 缺 persistence ref（内存模式）
或 self.md 不存在时，仅 instructions 通道留白；\`<self object_id>\` 仍按 persistence
渲染——objectId 是稳定标记，与 self.md 是否存在解耦。
      `.trim(),
    },
