import type { Concept, DocNode } from "@meta/doc-types";
import { collaborable_v20260504_1 } from "@meta/object/collaborable/index.doc";
import * as stoneObject from "@src/persistable/stone-object";
import * as stoneData from "@src/persistable/stone-data";

/* ────────────────────────────────────────────────────────────────
 *  目录页：Relation 概念全貌
 * ──────────────────────────────────────────────────────────────── */

/**
 * Relation 概念：Object 与其他 Object 之间的长期关系信息。
 *
 * sources（关系数据落在 stone 持久化层的 knowledge/relations/ 目录）:
 *  - stoneObject — stones/{id}/ 目录骨架，承载 knowledge/relations/ 子树
 *  - stoneData   — stone 数据读写入口
 */
export type RelationConcept = Concept & {
  sources: {
    stoneObject: typeof stoneObject;
    stoneData: typeof stoneData;
  };

  /** 关系存储：peer 文件路径 + 作为 knowledge 激活 */
  peerFile: {
    title: string;
    summary?: string;
    pathConvention: DocNode;
    activationAsKnowledge: DocNode;
  };

  /** 有向 + 局部 + 允许不对称 */
  directedLocal: {
    title: string;
    summary?: string;
    directedness: DocNode;
    locality: DocNode;
    asymmetryAllowed: DocNode;
  };

  /** 没有全局索引；需要时按需聚合 */
  globalNote: {
    title: string;
    summary?: string;
    noGlobalIndex: DocNode;
    onDemandAggregation: DocNode;
  };
};

/* ────────────────────────────────────────────────────────────────
 *  数据填充
 * ──────────────────────────────────────────────────────────────── */

export const relation_v20260506_1: RelationConcept = {
  name: "Relation",
  get parent() {
    return collaborable_v20260504_1;
  },
  sources: { stoneObject, stoneData },
  description: `
Relation 描述 Object 之间长期关系信息的形成、更新与使用。
每个对象只维护自己看到的关系，没有中心化索引——更接近现实中"我对你的印象"。
`.trim(),

  peerFile: {
    title: "peerFile",
    summary: "stones/{selfId}/knowledge/relations/{peerId}.md 单文件结构",

    pathConvention: {
      title: "路径约定",
      content: `
\`stones/{selfId}/knowledge/relations/{peerId}.md\`——文件名 = peer 的 objectId。
没有 peer 文件时即代表"无关系记录"，不需要预创建空文件。
      `.trim(),
    },

    activationAsKnowledge: {
      title: "作为 knowledge 激活",
      content: `
\`relations/{peer}.md\` 是普通 knowledge，按 \`activates_on\` 在与 peer 的 talk
上下文中自动加载。LLM 看到的不是"关系表"，而是与对方对话时浮现的相关知识页。
      `.trim(),
    },
  },

  directedLocal: {
    title: "directedLocal",
    summary: "关系是有向、局部、允许不对称的",

    directedness: {
      title: "有向性",
      content: `
A 对 B 的关系文件与 B 对 A 的关系文件互不约束。A 说"B 是合作伙伴"不要求 B 也
持有对应文件，反之亦然。
      `.trim(),
    },

    locality: {
      title: "局部性",
      content: `
每个对象只持有自己视角下的关系——没有全局对称性约束。这与社交网络的"好友
关系"不同，更接近现实中的"我对你的印象"。
      `.trim(),
    },

    asymmetryAllowed: {
      title: "允许不对称",
      content: `
A 对 B 的评价与 B 对 A 的评价可以完全不同；系统不强行对齐两端内容。这条
约束让对象保有真实的个体视角。
      `.trim(),
    },
  },

  globalNote: {
    title: "globalNote",
    summary: "不存在中心化的全局 relation 索引",

    noGlobalIndex: {
      title: "没有共享 relation 表",
      content: `
任何对象都不能"查询所有关系"——这条规则等同于"对象不能拥有全局世界视角"。
      `.trim(),
    },

    onDemandAggregation: {
      title: "按需聚合",
      content: `
需要"跨对象的关系全貌"时，由调用方按需聚合各对象 \`relations/*.md\`，
而不是查一张共享表。聚合行为本身是独立 use case 而非数据原语。
      `.trim(),
    },
  },
};
