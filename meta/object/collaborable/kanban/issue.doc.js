...
新增关联时**两侧都要更新**——避免出现"A 引用 X 但 X 不知道"的不一致。
...
由 supervisor 通过 server 方法操作（详见 collaborable/role/supervisor）：
...
普通 Object 通过 talkable 下 issue-discussion 相关 knowledge 只能评论，不能改结构。

## 评论

详见 [comment](./comment.doc.js)。

## 写入路径与并发

涉及多方写入：supervisor / 其他 Object / user API。
通过 SerialQueue per-sessionDir 串行化。详见 [concurrent-write](./concurrent-write.doc.js)。
`,
};
