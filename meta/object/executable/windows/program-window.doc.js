...
match() 据此追加 path：exec / exec.shell / exec.ts / exec.js / exec.function。
    ...
    shell_v20260517_1: {
      title: "shell — language=\"shell\" + code=<shell>：起新系统 shell 进程执行 code，每次 exec 全新进程。详细约...",
      content: `#### shell — language="shell" + code=<shell>：起新系统 shell 进程执行 code，每次 exec 全新进程。详细约束见各子节点。`,

      timeoutAndTruncate_v20260517_1: {
        title: "timeoutAndTruncate — 30 秒超时（exit code 124）；stdout / stderr 各 4KB 截断；超过的部分直接丢弃...",
        content: `##### timeoutAndTruncate — 30 秒超时（exit code 124）；stdout / stderr 各 4KB 截断；超过的部分直接丢弃，避免淹没 context。`,
      },

      cwdAndEnv_v20260517_1: {
        title: "cwdAndEnv — cwd 固定为 process.cwd()，env 继承 parent process；额外注入 env OOC_SELF_DIR...",
        content: `##### cwdAndEnv — cwd 固定为 process.cwd()，env 继承 parent process；额外注入 env OOC_SELF_DIR 让 shell 中可定位当前对象目录。`,
      },

      noThreadLocal_v20260517_1: {
        title: "noThreadLocal — shell 之间**不**共享 thread-local 数据（OS 进程隔离）；需要跨 exec 共享数据请用 ts/j...",
        content: `##### noThreadLocal — shell 之间**不**共享 thread-local 数据（OS 进程隔离）；需要跨 exec 共享数据请用 ts/js + self.getThreadLocal/setThreadLocal。`,
      },
    },

    ts_v20260517_1: {
      title: "ts — language=\"ts\" / \"typescript\" + code：sandbox 内执行 ts；每次 exec 重新加载用户代码模块。详见...",
      content: `#### ts — language="ts" / "typescript" + code：sandbox 内执行 ts；每次 exec 重新加载用户代码模块。详见各子节点。`,

      reloadEachExec_v20260517_1: {
        title: "reloadEachExec — 每次 exec 都重新动态 import 用户代码模块，不缓存；避免上一次 exec 的副作用残留。",
        content: `##### reloadEachExec — 每次 exec 都重新动态 import 用户代码模块，不缓存；避免上一次 exec 的副作用残留。`,
      },

      threadLocalChannel_v20260517_1: {
        title: "threadLocalChannel — 可通过 self.getThreadLocal(key) / self.setThreadLocal(key, ...",
        content: `##### threadLocalChannel — 可通过 self.getThreadLocal(key) / self.setThreadLocal(key, value) 跨 exec 共享数据；数据存 thread.threadLocalData，是 ts/js mode 独有的能力（shell 不接此通道）。`,
      },
    },
