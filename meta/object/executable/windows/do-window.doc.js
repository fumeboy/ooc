...
`,
      ...
      execution_v20260517_1: {
        index: `#### execution（executeDoWindowContinue）— 顺序 4 步详见各子节点。`,

        step1Validate_v20260517_1: {
          index: `##### step1Validate — parentWindow 必须是 type=do；通过 findChild(thread, targetThreadId) 在子树中定位 child；任一不满足 → 返回 [do_window.continue] 错误。`,
        },

        step2DoubleWrite_v20260517_1: {
          index: `##### step2DoubleWrite — 构造 message（source="do"）：target.inbox 追加 + 事件 context_change.inbox_message_arrived；同时 thread.outbox 追加镜像消息让父线程能看到自己发了什么。`,
        },

        step3ReviveChild_v20260517_1: {
          index: `##### step3ReviveChild — 若 child.status 是 done/failed → 切到 running，让 scheduler 重新选中执行；保证子线程不会因为之前的 end 错过新消息。`,
        },

        step4OptionalWait_v20260517_1: {
          index: `##### step4OptionalWait — args.wait===true 时设置 thread.status=waiting / inboxSnapshotAtWait=inbox.length / waitingOn=parentWindow.id；省一次显式 wait tool 调用。`,
        },
      },
