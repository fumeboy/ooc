...
export const search_window_v20260516_1 = {
  ...
  fields_v20260517_1: {
    title: "fields",
    content: `search_window 的字段集合；详见各子节点。`,
...
但 grep kind 的 match 多带 line + snippet，影响 open_match 的 file_window 切片策略。
    ...
    query_v20260517_1: {
      title: "query — 触发本次搜索的 pattern / regex；只读，不能被 mutate。",
      content: `#### query — 触发本次搜索的 pattern / regex；只读，不能被 mutate。`,
    },
...
- line / snippet — 仅 grep kind 存在
    ...
    truncated_v20260517_1: {
      title: "truncated — matches 超过 200 上限时为 true，提示 LLM 用更精确 query 重新搜。",
      content: `#### truncated — matches 超过 200 上限时为 true，提示 LLM 用更精确 query 重新搜。`,
    },

    searchRoot_v20260517_1: {
      title: "searchRoot — 搜索的根目录；open_match 中用它解析 match.path 的相对路径。",
      content: `#### searchRoot — 搜索的根目录；open_match 中用它解析 match.path 的相对路径。`,
    },
  },
...
想换条件就 open(command="glob"|"grep") 重新搜；不提供 next_page / refine_query。
  ...
  commands_v20260517_1: {
    title: "commands",
    content: `search_window 注册 2 个 command。`,
...
这段文本作为一个 protocol KnowledgeWindow 注入到 context。覆盖 4 条提醒详见各子节点。
    ...
    truncate200_v20260517_1: {
      title: "truncate200 — matches 超过 200 条时截断，set search_window.truncated=true 提示 LLM 用更精...",
      content: `##### truncate200 — matches 超过 200 条时截断，set search_window.truncated=true 提示 LLM 用更精确 query 重新搜。`,
    },

    noRefineNewSearch_v20260517_1: {
      title: "noRefineNewSearch — 翻页 / 改 query 都通过新建 search_window，不提供 next_page / refine_q...",
      content: `##### noRefineNewSearch — 翻页 / 改 query 都通过新建 search_window，不提供 next_page / refine_query；与 immutability 约束一致。`,
    },

    grepLineSnippet_v20260517_1: {
      title: "grepLineSnippet — grep kind 的每条 match 带 line + snippet（单行 trim 200 字符），glob k...",
      content: `##### grepLineSnippet — grep kind 的每条 match 带 line + snippet（单行 trim 200 字符），glob kind 没有这两字段。`,
    },

    grepOpenMatchSlice_v20260517_1: {
      title: "grepOpenMatchSlice — open_match grep 自动 ±40 行切片（FILE_WINDOW_LINE_CONTEXT 常量），...",
      content: `##### grepOpenMatchSlice — open_match grep 自动 ±40 行切片（FILE_WINDOW_LINE_CONTEXT 常量），glob open_match 整体打开。`,
    },
  },
};
