...
但 grep kind 的 match 多带 line + snippet，影响 open_match 的 file_window 切片策略。
    ...
    query: {
      title: "query",
      content: `
      触发本次搜索的 pattern / regex；只读，不能被 mutate。
      `,
    },
...
- line / snippet — 仅 grep kind 存在
    ...
    truncated: {
      title: "truncated",
      content: `
      matches 超过 200 上限时为 true，提示 LLM 用更精确 query 重新搜。
      `,
    },
    ...
    searchRoot: {
      title: "searchRoot",
      content: `
      搜索的根目录；open_match 中用它解析 match.path 的相对路径。
      `,
    },
  },
...
这段文本作为一个 protocol KnowledgeWindow 注入到 context。覆盖 4 条提醒详见各子节点。
    ...
    truncate200: {
      title: "truncate200",
      content: `
      matches 超过 200 条时截断，set search_window.truncated=true 提示 LLM 用更精确 query 重新搜。
      `,
    },
    ...
    noRefineNewSearch: {
      ...
      content: `
      翻页 / 改 query 都通过新建 search_window，不提供 next_page / refine_query；与 immutability 约束一致。
      `,
    },
    ...
    grepLineSnippet: {
      ...
      content: `
      grep kind 的每条 match 带 line + snippet（单行 trim 200 字符），glob kind 没有这两字段。
      `,
    },
    ...
    grepOpenMatchSlice: {
      ...
      content: `
      open_match grep 自动 ±40 行切片（FILE_WINDOW_LINE_CONTEXT 常量），glob open_match 整体打开。
      `,
    },
  },
};
