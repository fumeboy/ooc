...
这是 Claude Code MultiEdit 的语义——让前置 edit 可以为后续 edit 制造唯一上下文。
        ...
        step4IoErrorPath: {
          ...
          content: `
          任何 read/write IO 错误 → 返回 [file_window.edit] <path>: <err> 作为 result；走 commandExecLifecycle 的 legacyErrorPrefix 失败路径。
          `,
        },
      },
      ...
      failureModes: {
        ...
        matchesMultipleTimes: {
          ...
          content: `
          "edit #<i>: oldString matches N times (must match exactly once)"：old 出现 N 次（>1），需扩展 old 上下文使其在当前 buffer 中唯一。
          `,
        },
        ...
        currentBufferSemantics: {
          ...
          content: `
          注意"当前 buffer"指**已应用前 i-1 项之后**，不是原始文件。这是 Claude Code MultiEdit 的语义，前置 edit 可以为后续 edit 制造唯一上下文。
          `,
        },
      },
