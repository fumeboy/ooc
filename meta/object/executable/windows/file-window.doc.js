...
- 渲染层在 renderFileWindowChildren 中按切片裁剪，32KB 截断
  ...
  commands_v20260517_1: {
    title: "commands",
    content: `file_window 注册 4 个 command。`,
...
LLM 错误恢复建议 edit.recoveryGuide、与其它写文件方式的对比 edit.alternativesComparison。
      ...
      applyAlgorithm_v20260517_1: {
        title: "applyAlgorithm（applyEdits）— 顺序 4 步详见各子节点。",
        content: `#### applyAlgorithm（applyEdits）— 顺序 4 步详见各子节点。`,

        step1ReadBuffer_v20260517_1: {
          title: "step1ReadBuffer — 从磁盘读 buffer（path = window.path），每次 edit 调用都重新读，不依赖渲染层缓存。",
          content: `##### step1ReadBuffer — 从磁盘读 buffer（path = window.path），每次 edit 调用都重新读，不依赖渲染层缓存。`,
        },
...
这是 Claude Code MultiEdit 的语义——让前置 edit 可以为后续 edit 制造唯一上下文。
        ...
        step3WriteFile_v20260517_1: {
          title: "step3WriteFile — 全部成功 → writeFile 覆写 path（utf8）。任一前置失败时整组不写盘，避免半成品。",
          content: `##### step3WriteFile — 全部成功 → writeFile 覆写 path（utf8）。任一前置失败时整组不写盘，避免半成品。`,
        },

        step4IoErrorPath_v20260517_1: {
          title: "step4IoErrorPath — 任何 read/write IO 错误 → 返回 [file_window.edit] <path>: <err> ...",
          content: `##### step4IoErrorPath — 任何 read/write IO 错误 → 返回 [file_window.edit] <path>: <err> 作为 result；走 commandExecLifecycle 的 legacyErrorPrefix 失败路径。`,
        },
      },

      failureModes_v20260517_1: {
        title: "failure Modes",
        content: `两类失败前缀都带 [file_window.edit] + 文件路径，便于 LLM 解析；详见各子节点。`,

        notFound_v20260517_1: {
          title: "notFound — \"edit #<i>: oldString not found\"：old 在当前 buffer 中完全不存在（可能空白 / 引号 /...",
          content: `##### notFound — "edit #<i>: oldString not found"：old 在当前 buffer 中完全不存在（可能空白 / 引号 / 行尾差异）。`,
        },

        matchesMultipleTimes_v20260517_1: {
          title: "matchesMultipleTimes — \"edit #<i>: oldString matches N times (must match exac...",
          content: `##### matchesMultipleTimes — "edit #<i>: oldString matches N times (must match exactly once)"：old 出现 N 次（>1），需扩展 old 上下文使其在当前 buffer 中唯一。`,
        },

        currentBufferSemantics_v20260517_1: {
          title: "currentBufferSemantics — 注意\"当前 buffer\"指**已应用前 i-1 项之后**，不是原始文件。这是 Claude Code...",
          content: `##### currentBufferSemantics — 注意"当前 buffer"指**已应用前 i-1 项之后**，不是原始文件。这是 Claude Code MultiEdit 的语义，前置 edit 可以为后续 edit 制造唯一上下文。`,
        },
      },
