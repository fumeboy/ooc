...
在 parent window 下创建 command_exec sub-window：

- parent_window_id 缺省 → ROOT_WINDOW_ID（root window 是隐含 virtual view）
- args 非空时立刻 apply 一次累积（commandPaths + accumulatedArgs）
- 满足 auto-submit 三条件（详见 \`commandExecLifecycle.autoSubmitRule\`）→ 立刻调 \`submit\` 返回结果
...
2. 调用 \`type.onClose({ thread, window })\`；返回 false 表示拒绝（如 creator window）→ 不删
3. 递归关闭所有 \`parentWindowId === window.id\` 的子 window（snapshot 避免迭代时修改）
4. 从 windows 表移除并释放 knowledge 引用计数
`.trim(),
    },
...
`.trim(),
    ...
    pathCollection_v20260517_1: {
      index: `
##### pathCollection（collectKnowledgePathsOf）

- command_exec：合并 \`commandKnowledgePaths\` + \`loadedKnowledgePaths\` + \`windowKnowledgePaths\`
- 其它 type：仅 \`windowKnowledgePaths ?? []\`
`.trim(),
    },
  },
