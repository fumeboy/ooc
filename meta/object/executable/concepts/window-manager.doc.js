...
`,
      ...
      parentDefaultsToRoot_v20260517_1: {
        index: `###### parentDefaultsToRoot — parent_window_id 缺省时挂到 ROOT_WINDOW_ID（root window 是隐含 virtual view，不需要显式创建）。`,
      },

      argsApplyOnCreate_v20260517_1: {
        index: `###### argsApplyOnCreate — opts.args 非空时立刻 apply 一次累积：accumulatedArgs = { ...args }，commandPaths = entry.match(args)；等价于 open 后立即 refine。`,
      },

      autoSubmitShortCircuit_v20260517_1: {
        index: `###### autoSubmitShortCircuit — 满足 commandExecLifecycle.autoSubmitRule 的 3 条件时直接 submit，跳过 LLM 第二轮显式 submit。`,
      },

      returnShape_v20260517_1: {
        index: `###### returnShape — 返回 { formId, autoSubmitted: boolean, submitResult? }；autoSubmitted=true 时附带 submitResult，否则只有 formId。`,
      },
    },
...
`,
    ...
    close_v20260517_1: {
      index: `##### close — 关闭任意 window；按顺序执行 4 步详见各子节点。`,

      lookupStep_v20260517_1: {
        index: `###### Step 1 — lookup window；不存在则直接返回 false（兼容 LLM 复述旧 id 的情况）。`,
      },

      onCloseRejectGuard_v20260517_1: {
        index: `###### Step 2 — 调用 type.onClose({ thread, window })；返回 false 表示拒绝（如 creator window / 合成 knowledge_window）→ 不删，立即返回。`,
      },

      cascadeChildren_v20260517_1: {
        index: `###### Step 3 — 递归关闭所有 parentWindowId === window.id 的子 window；先 snapshot 子列表再迭代，避免迭代过程中 mutate 导致漏关。`,
      },

      removeAndReleaseRefs_v20260517_1: {
        index: `###### Step 4 — 从 windows Map 移除该 window 并调 releaseKnowledgeRefs 释放它持有的 knowledge 引用。`,
      },
    },
...
`,
    ...
    pathCollection_v20260517_1: {
      index: `collectKnowledgePathsOf 按 window type 选择 path 集合；2 个分支详见子节点。`,
...
`,
  ...
  notResponsibleFor_v20260517_1: {
    index: `WindowManager **不**负责的 3 件事；详见各子节点。`,

    commandExecImpl_v20260517_1: {
      index: `##### commandExecImpl — 各 command 自身的 entry.exec 实现（由各 root/X.ts 与 windows/X.ts 提供）。`,
    },

    knowledgeEntries_v20260517_1: {
      index: `##### knowledgeEntries — knowledge entries 的具体内容（由 collectExecutableKnowledgeEntries 派生）。`,
    },

    persistence_v20260517_1: {
      index: `##### persistence — 持久化（由 src/persistable/thread-json.ts 处理）。`,
    },
  },
