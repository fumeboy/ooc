...
固定下发的协议常量集合。4 个子来源详见 protocol.subSources。
      ...
      subSources: {
        ...
        globalKnowledge: {
          ...
          content: `
          模块级 KNOWLEDGE 常量；任何 thread 都看到。
          `,
        },
...
匹配规则比对，命中即激活。loader 按 mtime 失效缓存（knowledge_window.reload 主要是语义提示）。
      ...
      presentation: {
        title: "presentation",
        content: `
        "full" | "summary"，对应渲染层不同的体积截断（knowledge 8KB）。
        `,
      },
    },
...
getWindowTypeDefinition(type).basicKnowledge 注入到 entries。
    ...
    step3ProtocolWindows: {
      title: "step3ProtocolWindows",
      content: `
      把 step1 + step2 的 entries 合成为 source="protocol" 的 KnowledgeWindow。
      `,
    },
...
explicit 优先级高，activator 同 path 跳过，避免 LLM context 中出现两份相同 knowledge 文本。
  ...
  pathPrefixConvention: {
    ...
    internalCommandsPrefix: {
      ...
      content: `
      各 command_exec 的 entry.knowledge() 派生 protocol entries。
      `,
    },
    ...
    internalWindowsPrefix: {
      ...
      content: `
      basicKnowledge 与 form input knowledge。
      `,
    },
