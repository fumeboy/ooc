...
export const knowledge_activation_v20260515_1 = {
  ...
  sources_v20260517_1: {
    title: "sources",
    content: `knowledge entries 的 3 类来源；详见各子节点。`,
...
固定下发的协议常量集合。4 个子来源详见 protocol.subSources。
      ...
      subSources_v20260517_1: {
        title: "sub Sources",
        content: `protocol 的 4 个子来源。`,

        globalKnowledge_v20260517_1: {
          title: "globalKnowledge — 模块级 KNOWLEDGE 常量；任何 thread 都看到。",
          content: `##### globalKnowledge — 模块级 KNOWLEDGE 常量；任何 thread 都看到。`,
        },

        rootCommandList_v20260517_1: {
          title: "rootCommandList — ROOT_KNOWLEDGE，描述 root window 上注册的全部顶层 command 与调用形态。",
          content: `##### rootCommandList — ROOT_KNOWLEDGE，描述 root window 上注册的全部顶层 command 与调用形态。`,
        },

        commandExecForm_v20260517_1: {
          title: "commandExecForm — 每个 command_exec 的 entry.knowledge(args, formStatus) 派生条目；按 ...",
          content: `##### commandExecForm — 每个 command_exec 的 entry.knowledge(args, formStatus) 派生条目；按 form 当前 args / status 动态变化。`,
        },

        windowBasicKnowledge_v20260517_1: {
          title: "windowBasicKnowledge — 每种 window type 在 registerWindowType 时注入的 basicKnowledg...",
          content: `##### windowBasicKnowledge — 每种 window type 在 registerWindowType 时注入的 basicKnowledge 字段；只要该 type 至少一个实例在场就合成。`,
        },
      },
    },
...
匹配规则比对，命中即激活。loader 按 mtime 失效缓存（knowledge_window.reload 主要是语义提示）。
      ...
      presentation_v20260517_1: {
        title: "presentation — \"full\" | \"summary\"，对应渲染层不同的体积截断（knowledge 8KB）。",
        content: `##### presentation — "full" | "summary"，对应渲染层不同的体积截断（knowledge 8KB）。`,
      },
    },
...
的 knowledge() 派生。
    ...
    step2WindowBasicKnowledge_v20260517_1: {
      title: "step2WindowBasicKnowledge",
      content: `
...
getWindowTypeDefinition(type).basicKnowledge 注入到 entries。
    ...
    step3ProtocolWindows_v20260517_1: {
      title: "step3ProtocolWindows — 把 step1 + step2 的 entries 合成为 source=\"protocol\" 的 Know...",
      content: `##### step3ProtocolWindows — 把 step1 + step2 的 entries 合成为 source="protocol" 的 KnowledgeWindow。`,
    },

    step4ActivatorWindows_v20260517_1: {
      title: "step4ActivatorWindows",
      content: `
...
activator 命中重复 path 时跳过（避免同 path 出现两份）。
  ...
  ephemeralVsPersisted_v20260517_1: {
    title: "ephemeral Vs Persisted",
    content: `合成结果的持久化策略；2 条子规则详见各子节点。`,
...
explicit 优先级高，activator 同 path 跳过，避免 LLM context 中出现两份相同 knowledge 文本。
  ...
  pathPrefixConvention_v20260517_1: {
    title: "path Prefix Convention",
    content: `protocol entries 的 key 用约定前缀区分来源（便于 debug 与去重）；3 类前缀详见各子节点。`,

    internalCommandsPrefix_v20260517_1: {
      title: "internal/commands/<name>/... — 各 command_exec 的 entry.knowledge() 派生 protocol...",
      content: `##### internal/commands/<name>/... — 各 command_exec 的 entry.knowledge() 派生 protocol entries。`,
    },

    internalWindowsPrefix_v20260517_1: {
      title: "internal/windows/<type>/... — basicKnowledge 与 form input knowledge。",
      content: `##### internal/windows/<type>/... — basicKnowledge 与 form input knowledge。`,
    },

    kernelPrefix_v20260517_1: {
      title: "kernel/... — globalKnowledge（KNOWLEDGE 常量）。",
      content: `##### kernel/... — globalKnowledge（KNOWLEDGE 常量）。`,
    },
  },
};
