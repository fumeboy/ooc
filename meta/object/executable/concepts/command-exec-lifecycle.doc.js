...
方括号内仅允许字母数字 / 下划线 / 点 / 中划线，避免误判普通 markdown 链接 [text](url) 为错误。
      ...
      commandErrorPrefix_v20260517_1: {
        title: "[command-error] — manager 在 catch 块包装异常时的固定前缀。",
        content: `##### [command-error] — manager 在 catch 块包装异常时的固定前缀。`,
      },

      legacyErrorWord_v20260517_1: {
        title: "[error] — 早期实现手写前缀；遗留代码使用，新 command 不应再写。",
        content: `##### [error] — 早期实现手写前缀；遗留代码使用，新 command 不应再写。`,
      },

      perCommandPrefix_v20260517_1: {
        title: "[<command>] / [<window>.<command>] — 各 command 自定义错误前缀（如 [file_window.edit] /...",
        content: `##### [<command>] / [<window>.<command>] — 各 command 自定义错误前缀（如 [file_window.edit] / [talk_window.say]），便于 LLM 识别错误来源。`,
      },
    },
...
详细规则见各子节点。
    ...
    multiWindowSharing_v20260517_1: {
...
这避免 LLM 关一个 form 把另一个还在用的 knowledge 提前撤出 context。
    ...
    autoRemoveReleasesRefs_v20260517_1: {
