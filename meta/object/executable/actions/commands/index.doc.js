...
| grep            | 按正则在文件内容里搜索 | 创建 search_window kind=grep（含 line+snippet）；后续可 open_match(index) |
    ...
    program: { index: `### program — 沙箱代码执行 / server 方法调用（详见 program.doc.js）` },
    talk: { index: `### talk — 跨对象 / 跨线程消息（详见 talk.doc.js）` },
    do: { index: `### do — 子线程派生 / 续写（详见 do.doc.js）` },
    plan: { index: `### plan — 线程计划文本（详见 plan.doc.js）` },
    todo: { index: `### todo — 可见待办与条件提醒（详见 todo.doc.js）` },
    end: { index: `### end — 标记线程任务结束（详见 end.doc.js）` },
    openFile: { index: `### open_file — 把文件引入 context（详见 open-file.doc.js）` },
    openKnowledge: { index: `### open_knowledge — 显式打开 knowledge doc（详见 open-knowledge.doc.js）` },
    writeFile: { index: `### write_file — 创建或覆盖文件并自动 spawn file_window（详见 write-file.doc.js）` },
    glob: { index: `### glob — 按文件名通配符查找（详见 glob.doc.js）` },
    grep: { index: `### grep — 按正则在文件内容里搜索（详见 grep.doc.js）` },
  },
