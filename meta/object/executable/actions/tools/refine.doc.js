...
2. 把 args 浅合并到 form.accumulatedArgs（同名键覆盖）
3. 重新调用 \`commandTable[command].match(accumulatedArgs)\` 计算激活路径
4. 若新增了路径（如初始 \`open(command=talk)\` 仅激活 \`talk\` 路径，refine 加了 \`context="continue"\` 后多激活 \`talk.continue\` 路径），增量激活对应的 knowledge
5. 若旧路径不再命中，对应仅本 form 引入的 knowledge 被卸载

不会触发 command 的 exec——执行只发生在 submit。
