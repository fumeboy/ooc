...
1. **如何进行行动**（actions）
    - tools：LLM 直接调用的 5 个原语（open / refine / submit / close / wait）+ mark 附加参数 + defer 特殊 command
    - commands：submit 触发的具体行动（program / talk / do / plan / compress / end）
    - 通过 form 把"参数填写"和"执行"分开，让复杂行动可分步思考

2. **如何编写后端方法**（server）
    - 每个 server 模块 export llm_methods / ui_methods 双通道
    - 始终注册（不参与激活管理），但 LLM 知不知道靠对应 knowledge 控制
    - sandbox 里通过 callMethod 调用；前端通过 HTTP call_method 调用

3. **如何编写前端界面**（client）
    - React 组件，给人看不给 LLM 看
    - server 的 ui_methods 是 client 唯一可调入口
    - 通过 callMethod / notifyThread 形成"用户操作 UI → Object 思考"闭环
