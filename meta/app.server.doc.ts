...
- modules: 按 feature 组织的路由模块（health / runtime / stones / flows / ui / issues / debug-ui）
...
当前模块清单: health / runtime / stones / flows / ui / issues / debug-ui。
...
- knowledge 目录与文件创建、更新（create-knowledge-directory / create-knowledge-file / put-knowledge-file）
...
route 命名与存储位置错位是 root-cause #3 的范围。
                    `.trim(),
                },
                flows: {
                    title: "flows",
...
flows 列表附带 session 的 paused 状态，供 web 直接展示与切换。
                ...
                ui: {
                    title: "ui",
                    content: `
为 web 提供 world / flows / stones 的只读浏览。
src/app/server/modules/ui/。

- GET /api/tree?scope=world|flows|stones&path=... — 服务端递归返回整棵目录树（非懒加载），
  过滤 dotfiles，并在一级 \`flows/{sessionId}\` / \`stones/{objectId}\` 节点打 \`marker=flow|stone\`。
  \`scope\` 是必填 query，缺失会被 schema 校验拒绝；service 内部据此选择 scopeRoot/scopePrefix。
...
ui 只做只读浏览，不负责 session、chat、stone knowledge 写入；写操作走
flows / stones / runtime。
                    `.trim(),
                    todo: [
