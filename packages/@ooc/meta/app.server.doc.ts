...
可切换的控制面能力，而不是让 web 去猜 engine 内部状态。
    ...
    named: {
        ...
        "window.methods": "Object 暴露给 LLM 调用的方法集合；不通过 HTTP 暴露",
    },
...
只有 ui_methods 走 HTTP 暴露，window.methods 不暴露。
            `.trim(),
            children: {
                health: {
                    title: "health",
...
- self / readme / data / server-source 的 get/put/patch
...
**🔥 HTTP stone 写直接 commit main（2026-06-09 重构）**：

所有写 stone 的 HTTP 操作（create-stone / put-self / put-readme / put-server-source）
**直接走 \`httpDirectMainWrite\`**：write → commitWorktree(main) → 立即生效。
不再有 metaprog worktree；LLM session 内写 stone 则落 session worktree（\`flows/<sid>\`），
由 super flow \`evolve_self\` 合入 main。

理由（设计哲学 - 契约 3：状态翻转唯一 owner）：
- HTTP 控制面改的是已审核内容，直写 main 是正确的权威入口，每次产生一个 commit。
- LLM session 写落 session worktree，由 evolve_self 闸门管控合入，隔离试验与权威。
- audit trail 完整：两路均有 git commit。

实现细节: src/app/server/modules/stones/versioning-helper.ts:wrapHttpWriteInWorktree（薄适配）→ src/programmable/versioning.ts:811 httpDirectMainWrite（write + commitWorktree on main）。
