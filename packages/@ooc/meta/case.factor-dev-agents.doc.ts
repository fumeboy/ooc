 ...
 * 2. 当任一 Agent 的 self/readme 协议变化、methods 增删、或 RPC 接入方式变化时, 本文档要同步。
    ...
    children: {
        ...
        atomic_decomposition: {
            ...
            **方案 A — 1 个大 Agent (sentry_factor_assistant)** (拒绝):
            - 把所有 15 个 skill 都堆进一个 stone 的 knowledge/, 一个 server/index.ts 30+ methods。
        ...
        agent_factor_dev: {
            ...
            **methods** (定义在 \`server/index.ts\` 的 \`window: ObjectWindowDefinition\`):
        ...
        skill_psm_query: {
            ...
            **psm-query 调的真实 HTTP**: AFS 检索接口 \`https://ecop.bytedance.net/api/governance_base/ecop/rpc/AgentAfs/AFSMethod\`,
