...
### 2.4 Prototype Chain = 代码复用

OOC 采用原型链继承而非类继承：

- 每个 ObjectDefinition 可选 `prototype` 字段指向另一个 ObjectType。
- 方法解析沿原型链向上，自身方法覆盖原型方法。
- 循环引用在注册时 fail-loud。

选择原型链而非类继承的理由：

1. Agent 场景下"运行时改自己的类定义"是常态（`programmable` 维度），原型链比类继承更容易动态修改。
2. Object 之间的差异往往是"在基础能力上加几个特化方法"（如 `sentry_factor_group` 在 `plan` 的基础上增加 `run_batch`），原型链的差异继承比分层类继承更自然。
3. 与 JavaScript/TypeScript 的运行时原生模型一致，实现成本低。
...
| **parent-child**（层级派生） | 我派生出你。父对象拥有子对象的生命周期。 | `do_w_abc` 派生的 `command_exec` form |
...
- Runtime Object 由 thread 在运行时按需创建（`do_window` 派生 `command_exec`、`open_file` 创建 `file` object）。
...
### 5.2 Prototype Chain vs. Composition 的取舍

**现状**：ooc-6 选择了原型链继承。

**潜在问题**：
- 原型链是单继承，但 Agent 能力的组合往往是多源的（一个 Object 可能同时需要 file 的读写能力 + plan 的任务拆解能力）。
- 原型链在运行时动态修改后，debug 会很困难（"这个方法到底从哪来的？"）。

**优化方向**：
- 引入 Mixin / Trait 机制：Object 可以声明多个 `mixins: ["file_ops", "task_tree"]`，注册时将 mixin 的方法合并到自身。Mixin 不参与 identity，只提供能力。
- 保留 prototype 用于 identity 继承（is-a），mixin 用于能力组合（has-a）。
- 在 observable 维度加入"方法来源追踪"：LLM 调一个 method 时，context 中能看到它是从哪个 mixin / prototype 来的。
...
```
  ...
  身份与结构     │  Stone（self.md,      │  Flow Object         │
                 │   prototype, mixin）  │  （identity + state）│
                 │                      │                      │
                 ├──────────────────────┼──────────────────────┤
                 │                      │                      │
...
- Stone → Flow Object：原型链 + mixin 决定实例有哪些方法。
