...
- **identity vs pool 双轨（有意区分，别混淆）**：identity（self.md / readable / executable / visible / knowledge seed）走 stone git + session worktree 闸门——业务 session 改是试验态，须 evolve_self 才进 main、可隔离可丢弃；pool 知识/数据（`pools/<id>/knowledge|data`）**直写即时沉淀**、立刻跨 session 可见、无 commit/无回滚闸门。两类"持久化"对"什么是试验态"给相反答案是设计使然（pool 本为跨 session 即时积累），观察时分清落点。
