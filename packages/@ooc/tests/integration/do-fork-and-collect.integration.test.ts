...
describe.skipIf(!hasLlmEnv)("integration: do-fork-and-collect", () => {
  ...
  test("supervisor forks a sub-thread, waits for it, then ends", async () => {
    const root = await makeRootThread(
      tempRoot,
      [
        "请用 open(method=\"do\", title=\"统计 ts 文件\", args={ msg: '请用 program(language=shell) 跑 find src -type f -name *.ts | wc -l 然后 end', wait: true }) 派生子线程。",
        "等子线程完成后（你会从 waiting 醒来），用 open(method=\"end\") 结束父线程。",
