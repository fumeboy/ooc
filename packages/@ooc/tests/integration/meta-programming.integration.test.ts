  ...
  test("agent registers a method then calls it", async () => {
    ...
    const { inbox, events } = bootstrapInboxFromPrompt(
      [
        ...
        "",
        "协议要求：",
        "1) open(type=\"command\", method=\"run\", description=\"...\") 只创建 form，不等于执行。",
        "2) 业务参数必须放在 exec.args 或 refine.args，不要把 code/window_id/method 写进 description。",
        "3) 只有在参数齐全后，才 submit(form_id)。",
        "",
        "Step 1: 用 terminal 的 run method（bash）写文件，",
