  ...
  test("agent registers a method then calls it", async () => {
    ...
    const { inbox, events } = bootstrapInboxFromPrompt(
      [
        ...
        "",
        "协议要求：",
        "1) open(type=\"command\", method=\"program\", description=\"...\") 只创建 form，不等于执行。",
        "2) 业务参数必须放在 exec.args 或 refine.args，不要把 language/code/window_id/method 写进 description。",
        ...
        "",
        "Step 2: 用 exec(window_id='custom:agent', method='add', args={a:7,b:8}）调用方法。",
