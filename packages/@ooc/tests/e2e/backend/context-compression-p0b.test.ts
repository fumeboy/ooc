 ...
 * 3. dispatchToolCall(exec, method="expand", window_id=file_window_id)
...
describe("[p0b] context compression — compress(scope=windows) + expand 最小闭环", () => {
  it("compress → 压缩态 + 落事件;expand → 恢复 live + 再落事件", async () => {
    ...
    try {
      ...
      // ── 2. 压缩到 level=1（compress 经 exec 调用，不再是顶层 tool）
      ...
      // 2c. 断言渲染走 fallback 压缩态: 含 <compressed level="1"> + 就地 expand 提示,
      //     不含完整 <content> 节点
      ...
      // expand 不作为 <method name="expand"> 节点渲染（方法菜单已搬去 class 声明层）——
      // 压缩态就地贴 expand 提示（method="expand"）告诉 LLM 怎么恢复。
      expect(xmlCompressed).toContain(`method="expand"`);
