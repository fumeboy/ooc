 ...
 * （`executable.methods`〔LLM 命令面〕/ `visibleServer.methods`〔人类侧 UI〕数组，方法签名
 * `(ctx, self, args)`）定义自定义方法。visible/server 方法经 HTTP `call_method` 被人类侧调用，
 * 响应即标准 method result（ObjectMethodResult，结构化数据走 `data`）；改源码后热更新
 ...
 * `methods` = visibleServer 方法数组源码片段（人类侧 UI；含 name/description/exec(ctx,self,args)）。
 */
const M = (methods: string) => `export const Class = { visibleServer: { methods: ${methods} } };`;
/** LLM 命令面 object method（executable）数组源码片段——TC-PROG-03 验 loadStoneClass 加载。 */
const E = (methods: string) => `export const Class = { executable: { methods: ${methods} } };`;
...
export async function runControlPlane(): Promise<StoryResult> {
  ...
  try {
    // TC-PROG-01: visible/server 方法经 HTTP 调用，从 method result.data 取结果
    {
      const id = "echo_agent";
    ...
    // TC-PROG-02: visible/server 方法经 data 通道返回嵌套结构化数据（前端取数通道）
    {
    ...
    // TC-PROG-03: LLM 路径自定义 object method（executable）经 loadStoneClass 加载
    // （Wave4：loader 加载 `export const Class`，方法在 cls.executable.methods 数组里）
    {
      const id = "cmd_demo";
      await postJson(app, "/api/stones", { objectId: id });
      writeStoneFile(baseDir, id, "index.ts",
        E(`[{ name: "greet", description: "greet", intents: [{ name: "greet", description: "say hi" }], exec: async () => ({ message: "hi" }) }]`));
    ...
    // TC-PROG-04: 热更新 —— 改 visibleServer 后已有方法变更 + 新增方法立即生效
    {
      const id = "hot_prog";
      await postJson(app, "/api/stones", { objectId: id });
      writeStoneFile(baseDir, id, "index.ts",
