...
describe("splitOocText", () => {
  ...
  it("splits a bare flow uri out of surrounding text", () => {
    const r = splitOocText(
      "see ooc://client/flows/s1/alan/pages/report now",
    );
    expect(r).toEqual([
      { kind: "text", text: "see " },
      {
        kind: "ooc",
        uri: "ooc://client/flows/s1/alan/pages/report",
        route: "/flows/s1/alan/pages/report",
      },
      { kind: "text", text: " now" },
    ]);
  });
  ...
  it("keeps unrecognized ooc uri as plain text", () => {
    const r = splitOocText("nope ooc://client/flows/x/y here");
    expect(r).toEqual([
      { kind: "text", text: "nope ooc://client/flows/x/y here" },
    ]);
  });
