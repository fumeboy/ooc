/**
 * root.create_object method — 在业务 session 里建新对象骨架（落 session worktree）。
 */
...
骨架落 session worktree；end→super flow evolve_self 合入 main。`;
...
export async function executeCreateObject(ctx: MethodExecutionContext): Promise<string | undefined> {
  ...
  return JSON.stringify({
    ok: true,
    objectId: r.objectId,
    note: "已落 session worktree，end→super flow evolve_self 合入 main。",
  });
}
