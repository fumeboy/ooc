...
import { classifyStonesPath, resolveSessionPath } from "@ooc/core/extendable/_shared/session-path.js";
...
export async function executeWriteFileCommand(
  ...
  // packages/ 路径归属判定（2026-05-28 stones / 2026-06-01 bun workspace）：写 object 自治区
  // 必须经 versioning（git commit + self-scope ff-merge / cross-scope PR-Issue），不能裸 writeFile 绕过版本控制。
  // pools/ flows/ work/ 等非 packages 路径保持现状直写（运行时数据不进 git）。
  ...
  if (stoneClass.kind === "stone-object") {
    ...
    const versioned = await versionedStoneWrite({
      ...
      // 写入工作树：${worktree.path}/${relInObjects}（relInObjects 已含 packages/ 路径前缀）
      write: async (wt) => {
    ...
    if (!versioned.ok) {
      return `[write_file] versioning 写入失败 (${versioned.code})：${versioned.message}`;
    }
    // self-scope ff-merge 后工作区已反映新内容；cross-scope 落 PR-Issue 等 Supervisor 决议。
    ...
    return {
      ok: true,
      result: `[write_file] ${path} 经 versioning ${scopeNote}。`,
    };
  }

  if (stoneClass.kind === "stones-world") {
    // workspace-level 资源（如 packages/package.json，不在某个 object 子目录里）——LLM 不该
