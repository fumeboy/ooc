...
调用示例：
exec(method="end", description="结束当前线程")
refine(form_id, { reason: "done", summary: "end method 的 KNOWLEDGE 已补齐，测试通过" })
submit(form_id)

带 result（子线程把最终结果带回父）：
exec(method="end", args={ reason: "done", result: "分析完成：见 memo/x.md" })
`;
 ...
 * 调 continueMethod.exec 把 result 写入子→父 transcript，然后把 creator window 状态
...
/**
 * 在 creator talk_window 上模拟一次 say：调 sayMethod.exec 派送给 caller。
 *
