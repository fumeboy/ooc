...
/**
 * @deferred-hook compressView —— 契约外的现存 hook（new contract 尚无 compress 槽位）。
 *
 * 逻辑保留在本目录局部 helper（迁移期不丢），core 反推阶段把 compressView 接入新契约后再 wire。
 * 等级 1：读总行数 + read_range（来自 win.lines）；等级 2：仅总行数。两级都标 compressed 提示。
 */
export async function compressFileWindow(
  self: Data,
  win: FileWin | undefined,
  level: 1 | 2,
): Promise<XmlNode[]> {
  const attrs: Record<string, string> = { path: self.path };
