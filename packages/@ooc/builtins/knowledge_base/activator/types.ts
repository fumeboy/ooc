/**
 * Knowledge frontmatter 与索引类型 —— canonical 源（issue N: 从 core/types/knowledge.ts 迁入,
 * 因为 knowledge 激活机制整套下沉 builtins/knowledge_base）。零依赖纯类型。
 *
 ...
 *
 * **trigger 协议简化为单一 intent 维度**（issue N 裁决 5）—— 所有 trigger 都是 intent 形态：
 * `intent::<category>::<detail>`,详见 `./expr.ts`。
 *
