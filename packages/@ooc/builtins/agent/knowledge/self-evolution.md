...
activates_on:
  ...
  "intent::form_open::filesystem::write_file": "show_content"
---
...
- `stones/<self>/executable/index.ts` — **后端方法库 + 自定义 window**。export 的 `window.methods`
  ...
  与调 talk_window 同构。文件改了下一轮即生效。这是为自己**写工具**的入口。
...
- **身份/身体（stone：self.md / readable / executable / visible / seed knowledge）**：业务 session 里
  ...
  在 super 投影窗里调 `scan_changes` 扫描本 session 暂存的字段/源码改动 → 调 `create_pr_for_versioned`
  （版本化字段当前值起 PR）或 `create_pr_for_class_edits`（class 源码改动起 PR）一步到位 commit + 起 PR
  交 reviewer 评审、合入；非版本化字段直接 `sediment_unversioned` 写 pool。完整流程见 super flow 知识。
