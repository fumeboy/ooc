/**
 * Schema fill_state 构造 + 参数校验 —— form 填表态的纯逻辑（无实例状态）。
 *
 ...
 */
import type { MethodCallSchema, MethodArgSpec } from "@ooc/core/types";
import type { Data } from "./types.js";

export function buildFillState(
  schema: MethodCallSchema | undefined,
  args: Record<string, unknown>,
  existingFill?: Data["fill"],
): Data["fill"] | undefined {
  if (!schema) return undefined;
  const fill: NonNullable<Data["fill"]> = {};
  for (const [argName, spec] of Object.entries(schema.args)) {
    const hasValue =
      argName in args && args[argName] !== undefined && args[argName] !== null && args[argName] !== "";
