/**
 * transcript viewport —— thread 会话窗的 transcript 末 N 条 / 固定区间裁剪（本 class 自有）。
 *
 * 纯类型 + 纯函数：tail（末 N 条）与 range（固定区间）互斥。readable 的 set_transcript_window
 * window method 装配 mergeTranscriptViewport，投影时 applyTranscriptViewport 截取可见消息。
 */
