/**
 * terminal_process 的 history 投影 —— readable 投影纯函数 + set_history_window window method。
 *
...
/** terminal_process 窗的投影态（与 Data 分离）：history 视口。 */
...
/** window method：调本终端窗渲染的 history 视口（tail N 或固定 range）。 */
export const setHistoryWindowMethod: WindowMethod<Data, ProcessWin> = {
  ...
  exec: (_ctx: ReadableContext, _self: ReadonlySelfProxy<Data>, before: ProcessWin, args: Record<string, unknown>) => {
    ...
    if (!merged.ok) throw new Error(`[terminal_process.set_history_window] ${merged.error}`);
