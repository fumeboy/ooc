/**
 * 进程 window diff 组件 —— terminal_process / interpreter_process 共用。
 *
 * history 按 execId 配对：
...
/** 进程 window 的 diff 视图（fieldsTitle/historyTitle 由具体 class 注入文案）。 */
export function ProcessWindowDiff(
  { previous, current }: WindowDiffProps,
  labels: { testId: string; fieldsTitle: string; fieldsTestId: string; historyTestId: string },
) {
  ...
  return (
    <div data-testid={labels.testId}>
      <Section title={labels.fieldsTitle} testId={labels.fieldsTestId}>
      ...
      <Section title="history" testId={labels.historyTestId}>
