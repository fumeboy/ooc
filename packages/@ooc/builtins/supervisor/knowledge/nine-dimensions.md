...
| **collaborable** | 底座 | 协作：talk_window 通道（peer 跨 Object 会话 + fork 同对象子线程，消息 + 持续会话窗口） | 系统内核 |
...
- **不是 / 不确定 / 需要对方独立思考或跨 session 异步处理** → `exec(method="talk", args={ target: "<peer object>", title: "<主题>" })` 开 talk_window 转述需求
...
→ 拆解，并行派多个子 Object（用 talk(target=自己) fork 子 thread）。
