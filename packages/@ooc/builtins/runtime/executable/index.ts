/**
 * runtime —— executable 维度。
 *
 * runtime 是 agent 组合持有的 **tool-object 成员**，向 Agent 提供系统级接口。本 increment 落
...
// create_object 的 exec：建对象骨架落 session worktree。导出供测试直接驱动（runtime 是其唯一注册家）。
...
// runtime 类的单处声明：executable（methods）+ readable + 可见性 flag。tool-object 成员，parentClass:null。
