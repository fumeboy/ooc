...
_reg.register(AgentClass);
// pr：reviewer 评审窗（无父，class 天然有四 facet）。
_reg.register(PrClass);

// 各窗类型 builtin class（class 天然有四 facet；缺省无父、无 root 回退）。
_reg.register(KnowledgeClass);
_reg.register(FileClass);
_reg.register(TodoClass);
...
_reg.register(MethodExecFormClass);
_reg.register(SearchClass);
_reg.register(SkillIndexClass);
_reg.register(PlanClass);
_reg.register(TerminalProcessClass);
_reg.register(InterpreterProcessClass);
_reg.register(FilesystemClass);
_reg.register(TerminalClass);
_reg.register(InterpreterClass);
_reg.register(RuntimeClass);
_reg.register(KnowledgeBaseClass);
