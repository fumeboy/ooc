...
export async function collectExecutableKnowledgeEntries(
  ...
  // 4) peer/children Object 自动注入 → ContextWindow (2026-05-28 ooc-6 Phase 6)
  //    替换原 relation_window 机制: peer Object 本身作为 context window 进入 context,
  //    通过自身 type 注册的 readable/readme 渲染。
...
export async function deriveRelationWindow(
  ...
  if (selfId !== "user") {
    try {
      ...
      for (const peer of [...siblings, ...children]) {
        ...
        if (peer === "user") continue; // user 是 passive object,不作为 Agent peer
  ...
  for (const [peerId, createdAt] of peerEarliest) {
    ...
    const selfSessionPath = `flows/${sessionId}/${selfId}/knowledge/relations/${peerId}.md`;
 ...
 * 3. 为每个 peer 创建一个 type=peerId 的 window，id=peerId
 * 4. 自身注册的 dispatcher 会自动路由到 peer 的 readable/readme/renderXml
 *
...
export async function derivePeerObjectWindows(
  ...
  for (const [peerId, createdAt] of peerEarliest) {
    ...
    try {
      ...
      // 动态加载 peer 的 window definition 并注册到 registry（new ooc-6 design）
