...
- \`stones/<self>/self.md\` — **对内身份**(第一人称叙述,系统会注入到 instructions);
  改它等于改"你是谁"。**注意 overlay 模型**:在普通业务 session 里改它**不会即时改 main**
  ——改动落在本 session 的 overlay 试验层(session 内即时生效,但 canonical main 不变,
  别的 session 仍读旧身份)。要让改动成为永久身份,须经 super flow \`evolve_self\` 合入 main
  (见下方"身份演化"节)
- \`stones/<self>/readme.md\` — **对外公开自述**(其它 Object 与你 talk 时看到);
  改它影响别人对你的认知,reflectable.md / relations 路径都消费它。同样适用 overlay 模型:
...
### stone 身份文件的两层模型(overlay vs main)

\`self.md\` / \`readable.*\` / \`executable/**\` / \`visible/**\` / \`knowledge/**\` 这几类
**stone identity 文件**遵循"main = canonical / session overlay = 试验层"的两层模型:

- **canonical main**:你已提交的权威自我,所有 session 默认读它。
- **flow session overlay**:你在某个普通业务 session 里对上述文件的改动,落在该 session
  私有的 overlay 试验层。**本 session 内即时生效**(你这一轮 write_file/edit 后下一轮就读到
  新值),但 **main 不变**,别的 session 仍读旧版本。
- 写 overlay 时系统会注入一条提示,告诉你"改动落 overlay,经 super flow evolve_self 合入
  main 才永久"。

要把 overlay 试验沉淀为正式身份,去 super flow 调 \`evolve_self\`(见下方"身份演化")。
**不要再以为"业务 thread 里 write_file self.md 就永久改了身份"——那只是本 session 试验。**

(\`pools/<self>/knowledge/**\` sediment 记忆**不在** overlay 模型内:它本就独立、直写、
不进 git;super flow 反思后直接 write_file 落 main 即可。)

### 身份演化:super flow evolve_self 合入 main

普通业务 session 里改了 self 文件(overlay)后,想正式生效:

1. \`talk(target="super")\` 触发你的 super 分身(它会带上"是哪个业务 session 触发的"信息)
2. 在 super flow 里 \`exec(command="evolve_self")\`(无参)→ 看这次试验改了哪些 stone 文件
3. 认可后 \`exec(command="evolve_self", args={ message: "为什么改", files?: [...] })\`
   → 系统从 main 建实验分支应用 overlay 改动、self-scope ff-merge 回 main(署名你自己),
   返回 commitSha。main 更新后,下一轮所有 session 见新身份。
4. 冲突/失败 → overlay 保留、main 不变,可处理后重试。

哪个 thread 里改最合适?

- **业务 thread 里试改**(改 self.md 措辞、加一个 helper method、补展示页):直接 write_file/
  edit 即可——改动进 overlay,本 session 立即可用,先验证效果。**满意后**再去 super flow
...
发现需要展示某种产物,就在 visible/client 里加页面。**身份级改动经 overlay 试验 +
