...
test("talk 投影窗含 talk window method (set_transcript_window) + 会话 method (say)", () => {
  ...
  expect(set!.methodNames).toContain("say");
});
 ...
 * say 的 msg 必填 → 出现为 <arg>；say 无其它参数 → 不混入无关 arg。
 */
test("required args rendered as <arg> under <method> (say.msg eager required)", () => {
  const set = computeVisibleMethodSet("_builtin/agent/thread", "talk", bizThread, builtinRegistry);
  expect(set).not.toBeNull();
  // 找到 say 这个 method 节点
