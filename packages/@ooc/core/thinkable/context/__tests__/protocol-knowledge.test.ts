...
// agency（end）跑在 agent 的 self 窗上（class 链含 `_builtin/agent`），end form 挂在它上面。
// end-reflection 的 activates_on 键 `method::_builtin/agent::end` 经类链匹配命中。
const AGENT_SELF_WIN = {
  id: "self_agent",
...
describe("root builtin knowledge content（砍机制留协议后的关键协议仍在）", () => {
  it("super-flow 含 sediment frontmatter 写作协议", async () => {
    ...
    expect(sf?.body).toContain("create_pr_and_invite_reviewers");
  });
  ...
  it("end-reflection 含 super 沉淀引导且是 hint 非 gate", async () => {
    ...
    // 不在 end hint 里铺开——commit b9e47fac 微调后的口径）。
    expect(er?.body).toContain("长期记忆");
