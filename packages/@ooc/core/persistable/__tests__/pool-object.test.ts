...
describe("pool-object: 路径函数 + createPoolObject", () => {
  test("路径都挂在 pools/<id>/ 下（pool 不挂 branch，无 objects/ 中间层）", () => {
    const ref = { baseDir: "/abs/world", objectId: "agent" };
    const expectedRoot = join("/abs/world", "pools", "agent");
...
describe("derivePoolFromThread", () => {
  test("从 ThreadPersistenceRef 派生 PoolObjectRef（字段精简）", () => {
    const threadRef = {
      baseDir: "/abs/world",
      sessionId: "s1",
      objectId: "alice",
    ...
    // 显式断言 pool 不带 thread-only 字段
    expect("sessionId" in poolRef).toBe(false);
    expect("threadId" in poolRef).toBe(false);
  });
});
