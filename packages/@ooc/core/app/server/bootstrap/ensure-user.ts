/**
 * ensureUserObject — World bootstrap invariant：确保 user object package 存在。
 *
 * 设计动机（2026-05-25）：
 *   `user` 是真人用户在 OOC World 中的占位 Object，其 `readable.md` 定义了
 *   Object → user 消息的 inline UI token 协议（详见 `user-seed.ts`）。其它
 *   Object 通过读 user 的 readable（在 relation_window 中）学到这套语法。
 *
 *   把它升格为 bootstrap invariant 后：
 *   1. 第一启动自动建 user package（空 self.md + 真实 readable.md，无 seed knowledge）
...
/**
 * 检查 user package 是否已存在（通过 `package.json` marker，与 supervisor 同款判定）。
 */
async function userStoneExists(baseDir: string): Promise<boolean> {
...
/**
 * 第一启动时创建 user package。
 *
 * 流程（与 ensureSupervisorObject 同款，去掉 self.md 与 seed knowledge）：
 *   1. createStoneObject 预创 package.json + 空 self.md + 空 readable.md
 *   2. writeReadme 覆盖空 readable 为真实内容（self.md 保持空——user 无 LLM instructions）
 */
async function createUserStone(baseDir: string): Promise<string | undefined> {
  const ref = { baseDir, objectId: USER_OBJECT_ID };
...
/**
 * 启动期入口：确保 user package 存在；idempotent。
 *
...
export async function ensureUserObject(opts: {
  ...
  let created = false;
  if (!(await userStoneExists(opts.baseDir))) {
    commitSha = await createUserStone(opts.baseDir);
    created = true;
  }
