 ...
 * - method_exec form 成功执行后已自动消失，无需 close；本 tool 用于 close 失败 form / do_window /
 *   todo_window
 */
...
export const CLOSE_TOOL: LlmTool = {
  name: "close",
  description:
    "关闭一个 ContextWindow（form / do_window / todo_window）。必填 window_id 与 reason。关闭 do_window 等同于归档对应子线程；某些系统 window（如 creator do_window）会拒绝关闭。",
  inputSchema: {
    type: "object",
    properties: {
      title: TITLE_PARAM,
