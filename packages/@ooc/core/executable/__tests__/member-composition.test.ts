 ...
 *
 * 证明端到端：agent thread → injectMemberWindowsIfObjectThread 注入全局单例 tool-object
...
test("组合：agent thread → injectMember 注入 filesystem member 窗（inst.class=_builtin/filesystem，isMemberWindow 落 win）", async () => {
  ...
  try {
    ...
    const fsWin = thread.contextWindows.find(
      (w: OocObjectInstance) => w.object.class === FILESYSTEM_MEMBER_ID,
    );
...
test("组合机制命门：execObjectMethod(filesystem, grep) 经成员委托方法造出 search 对象（含真实命中）", async () => {
  ...
  try {
    ...
    const search = mgr.list().find((w) => w.object.class === "_builtin/filesystem/search");
    expect(search).toBeDefined();
    // 业务字段落 inst.object.data（Wave 4 实例）。
    const data = search!.object.data as { kind: string; matches: unknown[] };
...
test("非持久化往返：member 窗不入 thread-context.json；inline 会话窗落盘，readThread 重注入 member 窗", async () => {
  ...
  try {
    ...
    const talkWindow: OocObjectInstance = {
      id: "w_talk_x",
      parentObjectId: "root",
      title: "t",
      status: "open",
      createdAt: Date.now(),
      object: { class: THREAD_CLASS_ID, data: { target: "user", targetThreadId: "root" } },
    };
    thread.contextWindows = [talkWindow];
    await injectMemberWindowsIfObjectThread(thread);
    expect(
      thread.contextWindows.some((w: OocObjectInstance) => w.object.class === FILESYSTEM_MEMBER_ID),
    ).toBe(true);
    ...
    // injectMemberWindowsIfObjectThread）→ 再次可见、可 exec；会话窗 inline 整窗 hydrate 回。
    const reread = await readThread({ baseDir, sessionId: "_test_comp", objectId: "supervisor" }, "root");
    expect(
      reread?.contextWindows?.some((w: OocObjectInstance) => w.object.class === FILESYSTEM_MEMBER_ID),
    ).toBe(true);
    expect(
      reread?.contextWindows?.find((w: OocObjectInstance) => w.id === "w_talk_x")?.object.class,
