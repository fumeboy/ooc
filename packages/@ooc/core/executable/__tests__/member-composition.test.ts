...
test("非持久化往返：member 窗不入 thread-context.json；inline 会话窗落盘，readThread 重注入 member 窗", async () => {
  ...
  try {
    ...
    await injectMemberWindowsIfObjectThread(thread);
    expect(
      thread.contextWindows.some((w: OocObjectInstance) => w.class === FILESYSTEM_MEMBER_ID),
    ).toBe(true);
    ...
    const persistedEntries = (persisted.contextWindows ?? []) as OocObjectInstance[];
    const persistedClasses = persistedEntries.map((w) => w.class);
    expect(persistedClasses).not.toContain(FILESYSTEM_MEMBER_ID);
    // 会话窗（thread 实例）inline 持久化整窗落盘：entry 在、inst.class=THREAD_CLASS_ID 整窗保留。
    ...
    expect(
      reread?.contextWindows?.some((w: OocObjectInstance) => w.class === FILESYSTEM_MEMBER_ID),
    ).toBe(true);
    expect(reread?.contextWindows?.find((w: OocObjectInstance) => w.id === "w_talk_x")?.class).toBe(
      THREAD_CLASS_ID,
    );
  } finally {
    await rm(baseDir, { recursive: true, force: true });
