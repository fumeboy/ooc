...
describe("loadObjectWindow", () => {
  ...
  test("loads window.methods (for_ui_access) from executable/index.ts", async () => {
    tempRoot = await mkdtemp(join(tmpdir(), "ooc-srv-"));
    const ref = await createStoneObject({ baseDir: tempRoot, objectId: "x" });
      ...
      `export const window = {
        methods: {
          submit: {
            description: "submit",
            for_ui_access: true,
            exec: async ({ args }) => ({ ok: true, data: args.value }),
          },
        },
      };`
    );
    ...
    const win = await loadObjectWindow(ref);
    expect(Object.keys(win?.methods ?? {})).toEqual(["submit"]);
    expect(win?.methods?.submit.for_ui_access).toBe(true);
  });
});
