/**
 * root.plan command — 在当前 thread 上创建 / 更新 root plan_window。
 *
 ...
 *
 * 调用形态：
 * - exec(command="plan", args={ plan: "<text>" }) → 兼容旧调用：把 plan 文本写入 description
 * - exec(command="plan", args={ title, description?, steps?: PlanWindowStep[] }) → 完整指定
 *
 * 幂等性：
...
本命令会就地 update；否则会新建一个。返回值是该 plan_window.id。
