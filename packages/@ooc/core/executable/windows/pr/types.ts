 ...
 *
 * reflectable 沉淀（feat-branch PR 流程，spec 2026-06-11 §1 step5-7、§3 P4）：
 ...
 * request_changes）亲手批，底层走 P3 的 approvePrIssue 聚合 + P5 prAutoMerge 闸。
 *
 * 设计要点（不发明新协议层，复用既有机制）：
...
export interface PrWindow extends BaseContextWindow {
  ...
  /** 发起沉淀的 author（super(foo) 的 foo）；reject/request_changes 时 P6 message 回投的目标。 */
  authorObjectId: string;
  /** author 发起沉淀时所在的业务 session（P6 message 回投定位 super(foo) thread）。 */
