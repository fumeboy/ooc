...
func TestRegistryConversationLifecycle(t *testing.T) {
	...
	if err := reg.AttachAction(convID, &ActionState{MethodName: "test"}); err != nil {
		t.Fatalf("attach action failed: %v", err)
	}
	updated, ok := reg.GetConversation(convID)
	if !ok || len(updated.ActionIDs) != 1 {
		t.Fatalf("action id should be tracked")
	}
}
