 ...
 *
 * 职责：
 * - 维护写给 LLM 的全局 KNOWLEDGE 文本（系统机制 / window 类型 / 工具调用规则）
 * - 派生 command_exec form 关联的协议 knowledge（form.knowledge() 与 program function 签名）
 * - 把 thread.contextWindows 与 protocol / activator 来源的 KnowledgeWindow 一起合成
 *   返回给渲染层 / 前端
 */
...
你刚完成（或暂时无法推进）当前任务时，按下面顺序挑**唯一一个**收尾动作——
跳过应做的回复 / 结束直接 wait 是退化行为，等同于"任务做完但没人知道"：

1. **若 thread 是 callee（contextWindows 含 isCreatorWindow=true 的 talk_window）：必须先回复创建者**
   通过那个 creator talk_window 的 \`say\` 发一条结果消息（做了什么 / 结论 / 是否需要进一步信息），
   再决定 wait 还是 end。直接 wait/end 而没有 say，对面看不到任何结果。
2. **若任务真正完成、不会再有后续 inbox**（典型：自驱 root thread / fork 出来汇报上级的子线程）：
   用 \`end\` command 收尾，带 summary。这会结束 thread。
3. **仅当上述都不适用、明确在等下一条 inbox 消息推进时**：才用 \`wait(reason="...")\`。
...
export async function collectExecutableKnowledgeEntries(
  ...
  //      防止类似"看到 talk_window 但只会试 root 上的 talk command"的常见误用。
