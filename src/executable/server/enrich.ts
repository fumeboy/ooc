/**
 * server/enrich — 给 command_exec form 补充 command knowledge path 列表。
 *
 * 这个文件曾经叫 enrichProgramForm，名字保留是因为最早只服务 program 模式；
 * 现在它是通用的 command_exec 知识 enrich 入口，所有 command 都会走过来。
 */
 ...
 */
export async function enrichProgramFormCommand(
