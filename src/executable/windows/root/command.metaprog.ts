...
server / knowledge）通过这个命令族协调，错了能回退、合并需评审。
...
- \`create_object\` — 仅 supervisor 可调；为新 Object 原子地落盘 stone 骨架 + commit on main
  - args: { objectId: "<newId>", selfMd: "...", readmeMd: "...", knowledge?: { "<file>.md": "..." }, intent?: "..." }
  - 等价于"open_worktree → write → commit → merge（cross-scope→PR-Issue→supervisor 自审 merge）"的快捷路径——避免新 Object 创建产生自审 PR-Issue 噪音
  - 失败码：INVALID_INPUT（参数）/ ALREADY_EXISTS（stone 已存在）/ GIT
  - 结果：{ ok: true, objectId, commitSha }——文件已在 main 上 committed
...
3. **建议**：shell 启动一个独立 OOC Server 指向 worktree 做试运行验证
   \`bun run src/app/server/index.ts --world ./.ooc-world --stones-branch <branch> --port 0\`
4. \`open(command="metaprog", args={action:"commit", branch, intent:"..."})\`
5. \`open(command="metaprog", args={action:"merge", branch})\`
...
export async function executeMetaprog(ctx: CommandExecutionContext): Promise<string | undefined> {
  ...
  switch (action) {
    ...
    case "create_object": {
      if (callerId !== SUPERVISOR_OBJECT_ID) {
        return `[metaprog] create_object 仅 supervisor 可调（你是 ${callerId}）。`;
      }
      const newObjectId = asString(ctx.args.objectId);
      const selfMd = asString(ctx.args.selfMd);
      const readmeMd = asString(ctx.args.readmeMd);
      const intent = asString(ctx.args.intent);
      if (!newObjectId) return "[metaprog] create_object 需要 objectId。";
      if (!selfMd || !selfMd.trim()) return "[metaprog] create_object 需要 selfMd（非空字符串）。";
      if (!readmeMd || !readmeMd.trim()) return "[metaprog] create_object 需要 readmeMd（非空字符串）。";
      let knowledge: Record<string, string> | undefined;
      if (ctx.args.knowledge !== undefined) {
        knowledge = asStringMap(ctx.args.knowledge);
        if (!knowledge) {
          return "[metaprog] create_object 的 knowledge 必须是 { filename: content } 的 string map。";
        }
      }
      ...
      if (!r.ok) {
        if (r.code === "INVALID_INPUT") return `[metaprog] create_object 失败：${r.message}`;
        if (r.code === "ALREADY_EXISTS") return `[metaprog] create_object 失败：${r.message}`;
        return `[metaprog] create_object git 失败 (${r.gitCode})：${r.stderr}`;
      }
