...
import { doContinueCommand } from "./command.do-continue.js";
import { doCloseCommand } from "./command.do-close.js";
...
export const ROOT_METHODS: Record<string, MethodEntry> = {
  talk: talkCommand,
  do: doCommand,
  do_continue: doContinueCommand,
  do_close: doCloseCommand,
  program: programCommand,
  plan_set: planSetCommand,
  plan_clear: planClearCommand,
  todo_add: todoAddCommand,
  todo_check: todoCheckCommand,
...
| method          | 作用                                          | 主要副作用                                 |
|-----------------|-----------------------------------------------|--------------------------------------------|
| do              | 派生子线程                                    | 创建 child thread；子线程出现在 <self_view><active_children> |
| do_continue     | 向子线程追加消息 / 向 parent 线程回报；args={ target, content, wait? } | 写对端 inbox + 本 outbox；wait=true 进 waiting；子线程往返见 <self_view><active_children>，parent 回报口见 <self_view><parent_task> |
| do_close        | 归档一个你派生的子线程对话；args={ target } | 子线程切 paused、借出 window 自动归还；不再出现在 <self_view><active_children> |
| talk            | 向其它对象（含人类 user 与其它 flow object）发一条消息，与它持续会话；可选 wait 等回信 | 写 outbox + 派送对端 inbox + talks.json 路由；wait=true 进 waiting |
| program         | 执行代码 / 调用 server 方法                   | 创建 program_window；首次 exec 立即运行    |
| plan_set        | 全量设置对象级行动计划（markdown，步骤用 - [ ]/- [x] 自管） | 写 plan.md；非空计划常驻 self_view 切片 |
| plan_clear      | 清空对象级行动计划                            | 写空 plan.md；self_view 不再渲染 plan      |
| todo_add        | 登记一条对象级待办                            | 写 todos.json；未完成待办常驻 self_view 切片 |
| todo_check      | 把一条待办标记完成                            | 写 todos.json（done=true）；不再出现在自视  |
