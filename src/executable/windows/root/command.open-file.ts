/**
 * root.open_file method — 创建一个 file_window，把指定文件内容引入 context。
 *
...
后续操作：
- 调整范围：open(parent_window_id="<file_window_id>", method="set_range", args={ lines: [...] })
- 关闭：close(window_id="<file_window_id>")

调用示例：
open(method="open_file", title="读 README", args={ path: "README.md", lines: [0, 200] })
`.trim();
