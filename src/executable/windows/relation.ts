...
import { generateWindowId, type TalkWindow } from "./types.js";
...
root 上,要通过 open(parent_window_id="<rel_window_id>", command="edit", args={...}) 调用:

| command | 作用 | 典型用法 |
|---------|------|----------|
| edit    | 整文件替换 relation 文件;scope 决定写 session 层还是 long_term 层 | open(parent_window_id="<rel_window_id>", command="edit", args={ content: "...", scope: "session" }) |
...
- **long_term**:派一条 talk message 给 super flow,由 super 写
  \`stones/<self>/knowledge/relations/<peer>.md\`——跨 session 长期生效。适合"形成了
...
- scope:   必填,'session' | 'long_term'
  ...
  - long_term: 派一条 talk message 给 super flow,由 super 写 stones/<self>/knowledge/relations/<peer>.md(跨 session 长期生效)

典型用法(一步到位,args 齐时 open 立即提交):

  // 本 session 临时记下"该 peer 偏好简短回复"
  open(parent_window_id="<rel_window_id>", command="edit",
...
1. 本调用不直接写 stones/ 文件——它会派一条 talk message 到 super flow(self-reflection 分身);
2. super flow 会作为另一个 thread 收到这条消息,自行决定如何处理(典型:用 write_file 写 stones/.../relations/<peer>.md);
...
export async function executeRelationEdit(
  ...
  const composed =
    `请把我对 \`${peerId}\` 的长期 relation(stones/${selfId}/knowledge/relations/${peerId}.md)` +
    `更新为以下内容(整文件替换):\n\n---\n${content}\n---`;
  ...
  try {
    ...
    return `[relation.edit] 已派送 long_term relation 更新请求到 super flow(callee thread: ${result.calleeThreadId})。super 会写入 stones/${selfId}/knowledge/relations/${peerId}.md。`;
  } catch (error) {
