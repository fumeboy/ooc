/**
 * Method 相关类型 — 与 Window 抽象配套，但语义独立。
 *
 * - MethodEntry：单个 method 的完整定义（paths / match / knowledge / exec）
 * - MethodExecutionContext：method 的 exec 函数运行时入参
 * - MethodKnowledgeEntries：method.knowledge() 的返回 shape
 *
 * Window registry 中每种 type 的 \`methods\` map 由 MethodEntry 字典构成。
 *
 * Step 2 重构（spec 2026-05-14）后，本文件由 src/executable/commands/types.ts 移到
 * windows/ 目录下，以体现 "method 是 window 的能力" 这一从属关系。
 */
...
export interface MethodEntry {
   ...
   *
   * 返回 outcome 是首选；返回 string/undefined 兼容旧实现。详见 MethodExecOutcome 注释。
   */
  exec: (
...
/**
 * 方法执行上下文，由 WindowManager.submit 消费 form 后传入具体 method。
 *
 * 字段：
 * - thread：当前执行 method 的线程
 * - form：被 submit 消费的 form 自身（CommandExecWindow）
 * - parentWindow：method 注册到的 parent window；root method 时 parent.type="root"
