...
- root：每个 thread 隐含的根 window，注册了 do/talk/program/plan/end/todo_add/todo_check/.../open_file/open_knowledge 等顶层 method
...
- 让 thread 推进/结束：用 plan / todo_add / end 等 method 显式表达，不要依赖 message 文本
...
- 只是想记录一句话——直接 \`todo_add\` 或在 plan 里写下来，不必启动反思
...
1. **callee thread（contextWindows 含 isCreatorWindow=true 的 talk_window）**：
   - 完成任务后**必须先回复创建者一次**（不 say 就 wait/end，对面收不到结果）。
     精确调用形状（关键：用 say method 挂在已有 creator talk_window 上，**不要**
