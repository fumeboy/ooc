...
- LLM（你）通过 3 个原语作用在 window 上：
  ...
  - wait(on, reason?)：声明你在等指定来源上的未来 IO 事件，把 thread 切到 waiting。
    on 必填，可为：子线程 id（等子线程回报，见 <self_view><active_children>）/ creator talk_window id（等创建者）/
...
- root：每个 thread 隐含的根 window，注册了 do/do_continue/do_close/talk/program/plan_set/plan_clear/end/todo_add/todo_check/.../open_file/open_knowledge 等顶层 method
...
## 跨 thread 共享 ContextWindow（do 的 share_windows）

派生子线程时，可以把已有的 ContextWindow 一并带给子线程，让它直接看到你的上下文。
经 \`do\` 的 \`share_windows\` 参数指定，两种模式：

- **ref**（只读引用）：子线程获得分享时刻的 freeze snapshot；你保留 owner 继续 live 操作。
- **move**（移交所有权）：子线程获得完整 owner（live）；你这边临时只读，看分享时刻的 snapshot；
  子线程归档（do_close / 结束）后自动归还，你恢复 live。

调用形态（fork 时一次性带走多个 windows）：
\`\`\`
...
**自动归还**：do_close 归档子线程（或子线程结束）时，move 给它的 owner windows 自动归还你。
不可分享的类型：do / command_exec / root（语义不合理）。
...
- 在自己内部并行干活：用 \`do\` method 派生子线程。
  - \`exec(method="do", args={ msg: "<给子线程的初始任务>", wait: true|false })\` fork 一个子线程并下发任务。
  - 你派生的子线程出现在 \`<self_view><active_children><child thread_id=...>\` 自视切片里（含最近往返）。
  - 向子线程追加消息：\`do_continue(target=<子线程 id>, content="...", wait?)\`；归档子线程：\`do_close(target=<子线程 id>)\`。
  - 等子线程回报：\`wait(on=<子线程 id>)\`（子线程须仍 running）。
  - 如果**你自己**是被某个 parent 派生的子线程：parent 的任务与你的回报口出现在
    \`<self_view><parent_task parent_thread_id=...>\` 切片；向 parent 回报用
    \`do_continue(target=<parent 线程 id>, content="...")\`——这是子→父回报的唯一显式通道，不要塞进 end。
...
- \`stones/<self>/executable/index.ts\` — **后端方法库 + 自定义 ContextWindow**;
  ...
  调用,与调任意 root method 同构;ts/js sandbox 里
...
\`on\` 必须指向合法 IO 来源：子线程 id / creator talk_window id / talks.json peer objectId），
不再是兜底选项：
...
2. **自驱 root thread / fork 子线程已汇报完上级**：用 \`end\` 收尾，summary 写结论。
   这类 thread 没有任何会话 peer 也没进行中的子线程可指，\`wait\` 会被系统 reject。
3. **父线程派发子线程后等结果**：\`wait(on=<子线程 id>)\`（见 <self_view><active_children>），等子线程回报。
