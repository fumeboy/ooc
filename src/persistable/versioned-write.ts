 ...
 *
 *   ensureStoneRepo → openMetaprogWorktree → write(callback) → commitWorktree → tryMergeSelf
 *     - self-scope（diff 全落在 author 自治区 objects/<author>/）→ ff merge 回 main
...
export async function versionedStoneWrite(
  ...
  // 契约：本函数自己负责 stone repo init 前置条件，不依赖 caller 记得调用。
  // ensureStoneRepo 是 idempotent 的，已 init 时是 fast no-op。
  try {
