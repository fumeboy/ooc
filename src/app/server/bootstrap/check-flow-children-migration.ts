 ...
 * 解释为目录分隔符，sub-object 落在 `objects/<a>/<b>/`，与 stone 的 `objects/<a>/children/<b>/`
 * 不对称。本次统一为 children/ 嵌套。
 *
 * 启动时扫所有 session：把任何 `objects/<a>/<b>/` 形态（即非 children/ 子目录里又含
 * .flow.json 的目录）平移到 `objects/<a>/children/<b>/`。幂等：已在 children/ 下的不动。
 *
...
import { STONE_CHILDREN_SUBDIR } from "@src/persistable";
...
async function migrateSession(sessionObjectsRoot: string): Promise<number> {
  let entries;
  try {
    entries = await readdir(sessionObjectsRoot, { withFileTypes: true });
  } catch {
    return 0;
  }
  let moved = 0;
  for (const e of entries) {
    if (!e.isDirectory()) continue;
    const root = join(sessionObjectsRoot, e.name);
...
export async function checkFlowChildrenMigration(baseDir: string): Promise<{ migrated: number }> {
  ...
  try {
    ...
    for (const s of sessions) {
      ...
      total += await migrateSession(join(flowsDir, s.name, "objects"));
    }
  } catch (err) {
