...
import { objectDir, stoneDir } from "@src/persistable";
...
export function clientSourceUrlApi(config: Pick<ServerConfig, "baseDir" | "stonesBranch">) {
  return new Elysia({ name: "ooc.ui.api.client-source-url" }).get(
    ...
    async ({ params, query }) => {
      ...
      let absPath: string;
      if (scope === "stone") {
        absPath = join(stoneDir({ baseDir: config.baseDir, objectId, stonesBranch: config.stonesBranch }), "client", "index.tsx");
      } else {
